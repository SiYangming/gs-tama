...
# If no stop codon is found the reading frame is
# not reported (due to poly A tail selection)
#
    ...
    for i, amino_acid in enumerate(prot_seq):
        if amino_acid == "$":
            if i > 0:
                ...
                orf_end = i
                orf_obj = Orf(orf_seq, orf_start, orf_end, n_start, n_end,
                              frame)
...
def sort_orf_list(orf_list1, orf_list2, orf_list3):
    ...
    # get longest 4 orf's, may be less if there aren't that many due to the
