...
ap = argparse.ArgumentParser(description=(
    "This script produces a read support file for identifying what models "
    "are supported by which reads"), )

ap.add_argument(
    "-f",
    type=str,
    nargs=1,
    help="Filelist file with pre-merge trans_read.bed file names",
)
ap.add_argument(
    "-m",
    type=str,
    nargs=1,
    help=("""
        Merge.txt file from after merging.
        Use "no_merge" if there is no merge file.
    """),
),
...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help=("Ignore duplicate read name warning with -d dup_ok, "
          "default is to flag duplicates and terminate early."),
)
...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help=("Merge type flag indicates the type of merge file used. "
          "Use -mt cupcake for cupcake file. Default is TAMA output."),
)
...
if not opts.mt:
    ...
    if (merge_type_flag != "tama" and merge_type_flag != "cupcake"
            and merge_type_flag != "filter"):
...
source_trans_read_dict = {
}  # source_trans_read_dict[source][trans_id][read_id] = 1
source_list = []
    ...
    if file_type == "trans_read":
        ...
        for transread_line in transread_file_contents:
            ...
            for this_source_read_line in this_read_line_split:
                ...
                #############
                # use this to deal with nanopore read ID's with the ":" character in
                # them
...
# test_files/test_for_read_support_levels.txt
# print header
outline = "\t".join([
    "merge_gene_id",
    "merge_trans_id",
...
if merge_file != "no_merge":
    merge_trans_dict = (
        {}
    )  # merge_trans_dict[merge_trans_id][source name][source_trans_id] = read list
    merge_trans_list = []
        ...
        if merge_type_flag == "tama":
            ...
            if "trans_read.bed" in merge_file:
                ...
                if num_underscore_fields == 2:
                    ...
                    source_trans_id = source_trans_id_line.split("_")[
                        num_underscore_fields - 1]
                    source_name_underscore_list = []
                    for i in range(num_underscore_fields - 1):
                        source_name_underscore_list.append(
                            source_trans_id_line.split("_")[i])
            ...
            read_list = list(
                source_trans_read_dict[source_name][source_trans_id].keys())

            merge_trans_dict[merge_trans_id][source_name][
                source_trans_id] = read_list
            ...
            for source_trans_id in source_trans_id_list:
                ...
                read_list = list(source_trans_read_dict[source_name]
                                 [source_trans_id].keys())
            ...
            # test_files/test_filter.txt
            merge_trans_id = line_split[5]
            source_trans_id = line_split[1]
            merge_gene_id = line_split[4]

            # skip the transcripts that were removed in filtering
            if merge_trans_id == "removed_transcript":
                continue
    ...
    for merge_trans_id in merge_trans_list:
        ...
        this_source_read_dict = {
        }  # this_source_read_dict[source][read id] = 1
        this_read_source_dict = {
        }  # this_read_source_dict[read id][source] = 1
        this_source_list = []
        ...
        for source_name in source_list:
            ...
            if source_name in merge_trans_dict[merge_trans_id]:
                ...
                for source_trans_id in merge_trans_dict[merge_trans_id][
                        source_name]:
                    this_read_list = merge_trans_dict[merge_trans_id][
                        source_name][source_trans_id]
                    ...
                    for this_read_id in this_read_list:
                        ...
                        if len(list(this_read_source_dict[this_read_id].keys())
                               ) > 1:

                            if duplicate_flag == "no_dup":
                                source_list = list(
                                    this_read_source_dict[this_read_id].keys())
                                print("Error with duplicate read ID's from "
                                      "different sources")
        ...
        for source_name in source_list:
            if source_name in this_source_read_dict:
                this_source_read_list = list(
                    this_source_read_dict[source_name].keys())
            ...
            if source_name in gene_read_dict[merge_gene_id]:
                # get gene id and gene read support
                this_gene_read_num = len(
                    list(gene_read_dict[merge_gene_id][source_name].keys()))
    ...
    if source_name != "cluster":
        ...
        for merge_trans_id in source_trans_list:
            ...
            read_list = list(
                source_trans_read_dict[source_name][merge_trans_id].keys())
    ...
    for merge_trans_id in source_trans_list:
        ...
        num_read_support = len(
            list(source_trans_read_dict[source_name][merge_trans_id].keys()))
