...
ap = argparse.ArgumentParser(description=(
    "This script uses the TAMA collapse and TAMA merge outputs to remove "
    "Poly-A models"))
...
ap.add_argument("-f",
                type=str,
                nargs=1,
                help="Filelist file with Poly-A file names")
...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help=("Percent poly-A threshold (default of 75.0). "
          "Models with poly-A content above this threshold will be removed."),
)

ap.add_argument("-l",
                type=str,
                nargs=1,
                help="Level of removal (gene or transcript level)")
ap.add_argument(
    "-a",
    type=str,
    nargs=1,
    help=("Remove all models with Poly-A (all_polya or singleton_polya). "
...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help="Keep all multi-exon models (keep_multi or remove_multi)",
)
...
gene_trans_dict = {}
# gene_trans_dict[gene_id][trans_id] = trans_split

gene_trans_list_dict = {}
# gene_trans_list_dict[gene_id] = list of trans

trans_exon_dict = {}
# trans_exon_dict[trans_id] =  num exons
...
# test_files/trans_for_remove_polya_models_levels.txt

merge_trans_read_dict = {}
# merge_trans_read_dict[merge_trans_id][read id] = 1
merge_source_read_dict = {}
# merge_source_read_dict[merge trans id][source][read_id] = 1
    ...
    for this_source_read_line in this_read_line_split:
        ...
        if len(this_source_read_line_split) > 2:
            ...
            for i in range(len(this_source_read_line_split) - 1):
                new_this_source_read_line_split.append(
                    this_source_read_line_split[i + 1])

            new_this_source_read_line = ":".join(
                new_this_source_read_line_split)
...
source_polya_read_dict = {}
# source_polya_read_dict[source][read_id] = polya line split
# source_polya_trans_dict = {}
# source_polya_trans_dict[source][trans_id] = polya line
...
for line in filelist_file_contents:
    ...
    # test_files/example_tama_polya.txt
...
def write_support(this_merge_trans_id, this_support_source,
                  polya_support_list):
    outlist = []
...
# write out header
# m54041_170609_082817/20119745/30_408_CCS        G5.1    +       90.0
# 18      TAAAAAAAAAAAAGAAAAAA
outline = "\t".join([
    "trans_id",
    "source",
    "read_id",
...
for gene_id in gene_list:
    ...
    this_trans_num_reads_dict = {
    }  # this_trans_num_reads_dict[trans_id] = num reads
    ...
    ##################################
    this_trans_read_polya_support_dict = (
        {}
    )  # this_trans_read_polya_support_dict[trans_id][read_id] = polya support list
    ...
    for trans_id in trans_list:
        ...
        total_trans_num_reads = len(
            list(merge_trans_read_dict[trans_id].keys()))
        ...
        for this_source_name in this_source_list:
            ...
            this_read_id_list = list(
                merge_source_read_dict[trans_id][this_source_name].keys())
            ...
            for this_read_id in this_read_id_list:
                ...
                if this_read_id in source_polya_read_dict[this_source_name]:
                    ...
                    polya_percent = float(
                        source_polya_read_dict[this_source_name][this_read_id]
                        [3])
                    ...
                    if polya_percent >= polya_threshold:
                        ...
                        this_trans_read_polya_support_dict[trans_id][
                            this_read_id] = (
                                source_polya_read_dict[this_source_name]
                                [this_read_id])
    ...
    for this_trans_id in trans_list:
        ...
        if this_trans_id in this_polya_trans_dict:
            ...
            if filter_flag == "gene":
                if len(trans_list
                       ) > 1:  # keep if gene has multiple transcripts
                    this_keep_trans_dict[this_trans_id] = 1
                    continue
                elif len(trans_list) == 1:
                    if (multi_flag == "keep_multi"
                            and this_num_exons > 1):  # keep if multiple exons
                        this_keep_trans_dict[this_trans_id] = 1
                        continue

                    if (support_flag == "singleton_polya"
                            and this_trans_num_reads
                            > 1):  # keep trans if multiple reads
                        this_keep_trans_dict[this_trans_id] = 1
                        continue
                    ...
                    if support_flag == "all_polya":
                        ...
                        for this_read_id in this_trans_read_dict[
                                this_trans_id]:
            ...
            if filter_flag == "transcript":
                ...
                if (multi_flag == "keep_multi" and this_num_exons
                        > 1):  # multi flag is for keeping multiple exon trans
                    this_keep_trans_dict[this_trans_id] = 1
                    continue

                if (support_flag == "singleton_polya" and this_trans_num_reads
                        > 1):  # keep trans if multiple reads
                    this_keep_trans_dict[this_trans_id] = 1
                    continue
    ...
    for this_trans_id in trans_list:
        ...
        if this_trans_id in this_remove_trans_dict:
            ...
            if this_trans_id in this_trans_read_polya_support_dict:
                for this_read_id in this_trans_read_polya_support_dict[
                        this_trans_id]:
                    polya_support_list = this_trans_read_polya_support_dict[
                        this_trans_id][this_read_id]

                    this_support_source = this_read_source_dict[this_read_id]

                    write_support(this_trans_id, this_support_source,
                                  polya_support_list)
