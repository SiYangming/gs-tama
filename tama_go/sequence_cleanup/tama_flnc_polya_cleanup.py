...
ap = argparse.ArgumentParser(
    description="This script removes tailing poly A from FLNC fasta files")
...
ap.add_argument("-m",
                type=str,
                nargs=1,
                help="Minimum read length to keep (default is 200)")
...
class Block:
    ...
    def __init__(self, block_index, block_type, block_count, block_start,
                 block_end):
...
for seq_record in SeqIO.parse(fasta_file, "fasta"):
    ...
    for i in range(len(seq_string_list)):
        ...
        if seq_nuc == "A" and this_block_flag == "not_a_block":  # block change
            ...
            block_obj = Block(block_index, block_type, this_block_count,
    ...
    while stop_checking_flag == 0:
        ...
        if this_block_type == "a_block":
            ...
            if (this_block_index + 1
                    not in a_block_dict):  # check for running out of index
    ...
    if len(trim_seq_list) < min_length:
        ...
        outline = (">" + seq_name + "\t" + "short" + "\t" + "trimlen:" +
                   str(len(trim_seq_list)) + "\t" + "prelen:" +
                   str(len(seq_string_list)))
        ...
        outline = (">" + seq_name + "\t" + "overrun" + "\t" + "trimlen:" +
                   str(len(trim_seq_list)) + "\t" + "prelen:" +
                   str(len(seq_string_list)))
        outfile_filtered.write(outline)
        outfile_filtered.write("\n")
...
outline = "polya_upto_five_read_count:" + "\t" + str(
    polya_poly_five_read_count)

outfile_summary.write(outline)
outfile_summary.write("\n")

outline = "polya_sixormore_read_count:" + "\t" + str(
    polya_poly_sixmore_read_count)

outfile_summary.write(outline)
outfile_summary.write("\n")

outline = ("longest_trimmed_read_length:" + "\t" +
           str(longest_trimmed_read_length) + "\t" +
           "longest_trimmed_read_id:" + "\t" + str(longest_trimmed_read_id))

outfile_summary.write(outline)
outfile_summary.write("\n")

outline = ("peak_read_length:" + "\t" + str(peak_read_length) + "\t" +
           "peak_read_count:" + "\t" + str(peak_read_count))
