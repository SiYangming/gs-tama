...
# TODO: RK left off here; implement bed12 conversion
# Start of bed12 writing logic
...
for trans_id in trans_list:
    ...
    #            # correct for bed format conversion done in exon class
    #            e_start = e_start + 1
    #            e_end = e_ends[j]
    ...
    if cds_flag == 1:
        ...
        if strand == "+":
            ...
            bed_cds_end = cds_end
            """
            note that indexing here is not corrected for by
            bed format so we add a one for the later coord
            """
            # bed_cds_end = cds_end + 3 + 1
            # bed_cds_end = stop_codon_obj.ec_end + 1
            """
            note that indexing here is not corrected for by
            bed format so we add a one for the later coord
            """
            ...
            bed_cds_start = c_starts[
                0]  # Note that we are not counting stop codons here!!!
            bed_cds_end = c_ends[-1]

            # bed_cds_start = stop_codon_obj.ec_start
            # bed_cds_end = start_codon_obj.bc_end + 1 # note that indexing here is
            # not corrected for by bed format so we add a one for the later coord

        # rel_bed_cds_start = bed_cds_start - int(t_start)
        # rel_bed_cds_end = bed_cds_end - int(t_start) + 1 # note that indexing
        # here is not corrected for by bed format so we add a one for the later
        # coord
