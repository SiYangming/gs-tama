...
This script collapses transcripts and groups transcripts

into genes for long reads mapped onto a genome assembly.
...
ap.add_argument(
    ...
    nargs=1,
    help=("""
    Use error threshold to prioritize the use of splice junction information
    from collapsing transcripts(default no_priority, activate with sj_priority)
     """),
)
ap.add_argument(
    "-sjt",
    ...
    nargs=1,
    help=("""
    Threshold for detecting errors near splice junctions (default is 10bp)
    """),
)
ap.add_argument(
    "-lde",
    ...
    nargs=1,
    help=("""
    Threshold for amount of local density error near splice junctions
    that is allowed
     (default is 1000 errors which practically means no threshold is applied)
     """),
)
ap.add_argument(
    "-ses",
    ...
    nargs=1,
    help=("""
    Simple error symbol. Use this to pick the symbol used to represent matches in
    the simple error string for LDE output.
    """),
)
...
ap.add_argument(
    ...
    nargs=1,
    help=("""
    Run mode allows you to use original or low_mem mode, default is original
    """),
)
ap.add_argument("-vc",
                type=str,
                nargs=1,
                help="Variation covwerage threshold: Default 5 reads")
...
# trans_report_line = "\t".join([
#     "transcript_id","num_clusters","high_coverage","low_coverage",
#     "high_quality_percent","low_quality_percent",
#     "start_wobble_list","end_wobble_list",
#     "collapse_sj_start_err","collapse_sj_end_err",
#     "collapse_error_nuc","sj_error_simple"
# ])
...
#     "transcript_id","cluster_id","scaffold","strand",
...
# rtswitch_file_line = "\t".join(["trans_id","junct_num","first_seq",
...
def calc_error_rate(start_pos, cigar, seq_list, scaffold, read_id):
    ...
    for i in range(len(cig_dig_list)):
        ...
        if cig_flag == "H":
            ...
            #########################################
            # not related to sj error should probably move this up or down
            # code of this block
            #########################################
    ...
    # print(cigar)
    # blah = ",".join([
    #     str(h_count),str(s_count),str(i_count),
    #     str(d_count),str(mis_count)
    # ])
    # print(blah)
    # print(cig_dig_list)
    # print(cig_char_list)
...
class Merged:
    ...
    def format_trans_report_line(self):
        ...
        #
        #                sj_pre_error_simple_string, sj_post_error_simple_string =
        #                simple_sj_error(
        ...
        #
        #                all_sj_both_error_simple_list.append(
        #                    sj_both_error_simple_string
        #                )
        #
...
def sj_error_priority_finder(trans_obj, i, max_exon_num):
    ...
    if sj_priority_flag == "no_priority":
        ...
        print("Error with splice junction priority flag. "
            ...
            for pos_element in same_order_index_list:
                ...
                for this_pos in slice_pos_index_list:
                    if past_pos == "na":
                        ...
                        print(
                            "Error with non-consequtive indices for same pos in "
                            "trans sorting.")
    ...
    for trans_obj in trans_obj_list:
        ...
        if trans_pos_line in pos_trans_dict:
            ...
            if duplicate_flag == "no_merge":
                print("By default TAMA collapse does not allow merging of "
                      "duplicate transcript groups.")
                print("Duplicate transcript groups occur when different "
                      "groupings of transcripts results in the same "
                      "collapsed model.")
                print("If you would like to merge duplicate transcript groups "
                ...
                # collapse_start_list, collapse_end_list,
                # start_wobble_list, end_wobble_list,
                # e_start_trans_dict, e_end_trans_dict = collapse_transcripts(
                #     match_trans_obj_list, collapse_flag
                # )
                (
                    collapse_start_list,
                    collapse_end_list,
                ...
                #     collapse_start_list, collapse_end_list,
                #     start_wobble_list, end_wobble_list,
                #     e_start_trans_dict, e_end_trans_dict
                # )
                pos_trans_dict[trans_pos_line].add_merge_info(
                    collapse_start_list,
                    collapse_end_list,
    ...
    if strand == "+":
        if num_exons > longest_num_exons:
            ...
            if trans_start < longest_tss_pos:
                ...
                print("Error with trans_start/longest_tss_pos comparison "
...
class TransGroup:
    ...
    def delete_trans(self, trans_a):
        ...
        for a_trans_group in self.trans_group_dict[trans_a]:
            ...
            # if this group is now empty, delete it
            if len(list(self.group_trans_dict[a_trans_group].keys())) == 0:
    ...
    def add_a_to_b_group(self, trans_a, trans_b):
        ...
        for b_group_num in list(self.trans_group_dict[trans_b].keys()):
            ...
            # if a is longest in any of it's groups
            # then you can add other trans from those groups to b
    ...
    while ungrouped_count > 0:
        ...
        while unsearched_count > 0 and ungrouped_count > 0:
            ...
            for prey_cluster_id in ungrouped_trans_obj_dict:
                ...
                #    trans_comp_flag, start_match_list, start_diff_list, end_match_list,
                #    end_diff_list, short_trans, long_trans,
                #    min_exon_num, diff_num_exon_flag =
                #    compare_transcripts(
                ...
                #        trans_comp_flag = "same_transcript"
                #
                #        # exact match is the same as compare for
                #        # single exon transcripts
                #    elif hunter_num_exons == 1:
                #        trans_comp_flag = 'diff_transcripts'
                #
                #    elif exact_match_flag == "not_exact":
                #        trans_comp_flag, start_match_list,
                #        start_diff_list, end_match_list,
                #        end_diff_list, short_trans, long_trans,
                #        min_exon_num, diff_num_exon_flag =
                #        compare_transcripts(
                #            hunter_trans_obj, prey_trans_obj,
                ...
                if trans_comp_flag != "same_transcript":  # skip if there is no match
                    ...
                    # merge groups, applies to both caps
                    ...
                    for prey_cluster_id in ungrouped_trans_obj_dict:
                        ...
                        # if they are in different groups, but match
                            ...
                            if hunter_num_exons == prey_num_exons:
                                ...
                                # remove the prey cluster from dict
                                # to avoid redundant searching
                                # ungrouped_trans_obj_dict.pop(prey_cluster_id)
                                # remove this outside of for loop
                                # add prey to unsearched dict
...
if run_mode_flag == "original":
    for line in sam_file_contents:
        ...
        if (percent_coverage < coverage_threshold
            ...
            if bad_sj_flag > 0:
                ...
                # lde_file_line = "\t".join([trans_obj.cluster_id,trans_obj.scaff_name,
                # str(trans_obj.start_pos),str(trans_obj.end_pos),trans_obj.strand,
                # bad_sj_num_line,bad_sj_error_count_line,trans_obj.cigar])
                # outfile_lde.write(lde_outline)
                # outfile_lde.write("\n")
    ...
    for i in range(total_group_count + 1):
        ...
        if i not in group_trans_list_dict:
            ...
            print("This should only occur if you have a multi-map site "
        ...
        for gene_start in all_start_list:
            ...
            for trans_obj_list in gene_trans_obj_list:
                ...
                for match_group_num in match_group_trans_dict:
                    ...
                    match_trans_id_list = match_group_trans_dict[
                        match_group_num].keys()
                    match_trans_obj_list = []
...
def process_variation(scaffold, variation_dict, var_coverage_dict,
    ...
    for var_pos in position_list:
        ...
        # Use this to signal if a variation ahs passed threshold
        # for this position
