...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help=("Format of input DB ID (default is UniRef, "
          'use "ensembl" for Ensembl generated DB)'),
)
...
with open(blastp_file) as blastp_file_contents:
    for line in blastp_file_contents:
        ...
        if s_name_flag == 1:
            ...
            # >ENSGALP00000050219.1 pep chromosome:GRCg6a:MT:13071:14888:1
            # gene:ENSGALG00000029500.1
            # transcript:ENSGALT00000047001.1 gene_biotype:protein_coding
            # transcript_biotype:protein_coding
            # gene_symbol:ND5 description:NADH-ubiquinone
                ...
                if format_type == "uniref":
                    ...
                    for s_name_field in s_name_split:
                        ...
                        if s_name_field.startswith("transcript_biotype:"):
                            this_transcript_biotype = s_name_field.split(
                                ":")[1]
        ...
        # because the query name wraps around to new lines this is needed to
        # collect full names
        ...
        if passed_headers_flag > 0:
            ...
            print(trans_id + "\t" + "passed_headers_flag" + "\t" +
                  str(empty_line_count) + "\t" + str(hit_start_flag) + "\t" +
                  line)
            if empty_line_count > 1 and hit_start_flag == 1:
                ...
                trans_id_dict[trans_id][q_name].append(
                    Hit(
                        s_name,
                        q_name,
                        s_len,
        ...
        if line.startswith("Query="):
            ...
            hit_dict = {}  # hit_dict
            passed_headers_flag = (
                1  # used to flag when we have passed all the header info
            )
            query_id_line_flag = 1
        ...
        if line.startswith(">"):
            # test_files/sample_for_orf_blastp_parser.txt
        ...
        if line.startswith(" Identities"):
            ...
            ident_percent = (ident_line.split("/")[1].split()[1].split("(")
                             [1].split("%")[0])
...
for trans_id in trans_id_list:
    ...
    for q_name in trans_id_dict[trans_id]:
        ...
        trans_id = trans_id.split("(")[
            0]  # account for newer bedtools 2.27 formatting

        q_frame = q_name_split[-7]

        # for missing nucleotides the protein will not be complete so cannot do
        # ORF prediction
        if q_frame == "missing_nucleotides":
            best_match_line = "\t".join([
                trans_id,
                "no_frame",
                "-1",
                "-1",
        ...
        for s_obj in query_dict[q_name]:
            ...
            if int(s_align_percent) == 1:
                # minus 1 to adjust for coordinates
                # new_q_rel_start = s_obj.q_start - 1 + q_rel_start
            ...
            # If there is 90% match then use the start site of the protein and the end
            # position of the ORF
            ...
            # If there is 50% match then use the start site of
            # the protein match on the query and the end position of the ORF
            # If there is 50% match then use ORF start and end
            if s_align_percent >= 0.5:
                # new_q_rel_start = s_obj.q_start - 1 + q_rel_start # minus 1 to adjust
                # for coordinates
                new_q_rel_start = q_rel_start
                new_q_rel_end = q_rel_end
