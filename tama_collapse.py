...
# Robust version import: works when run as script or module
...
This script collapses transcripts and groups transcripts into

genes for long reads mapped onto a genome assembly.

Notes on changes
Fixed issue with coordinates of soft clipped variants in the variant output file.

Local Update: 2025/10/16

Modified by: Yangming Si

Notes: Python2→Python3 compatibility adjustments recorded in
  README.md (e.g. regex escape fixes, io.StringIO),
  and local version bump tc_version=1.0.4.

"""

# 本地兼容修复记录：Python2→Python3（详见 tama_go/README.txt）
tc_version = "1.0.4"
tc_date = "tc_version_date_2025_10_16"

ap = argparse.ArgumentParser(
    description="This script collapses mapped transcript models")

ap.add_argument("-s",
                type=str,
                nargs=1,
                required=True,
                help="Sorted sam file (required)")
ap.add_argument("-f",
                type=str,
                nargs=1,
                required=True,
...
ap.add_argument(
    ...
    nargs=1,
    help=("""
        Collapse exon ends flag: common_ends or longest_ends (default common_ends)
        """),
)
...
ap.add_argument(
    ...
    nargs=1,
    help=("""
        Identity calculation method (default ident_cov for including coverage)
...
ap.add_argument("-m",
                type=str,
                nargs=1,
                help="Exon/Splice junction threshold (default 10)")
...
ap.add_argument(
    ...
    help=("Flag for merging duplicate transcript groups "
          "(default is merge_dup will merge duplicates, "
...
ap.add_argument(
    ...
    nargs=1,
    help=("""
        Use error threshold to prioritize the use of splice junction information from
        collapsing transcripts(default no_priority, activate with sj_priority)
        """),
)
ap.add_argument(
    "-sjt",
    type=str,
    nargs=1,
    help=("Threshold for detecting errors near splice junctions "
          "(default is 10bp)"),
)
ap.add_argument(
    "-lde",
    ...
    nargs=1,
    help=("""
        Threshold for amount of local density error near
        splice junctions that is allowed
        (default is 1000 errors which practically
        means no threshold is applied)
        """),
)
ap.add_argument(
    "-ses",
    ...
    nargs=1,
    help=("""
        Simple error symbol. Use this to pick the symbol
        used to represent matches in the simple error string
        for LDE output.
        """),
)
...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help=("Turns off log output to screen of collapsing process. "
...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help=("Run mode allows you to use original or low_mem mode, "
          "default is original"),
)
ap.add_argument("-vc",
                type=str,
                nargs=1,
                help="Variation coverage threshold: Default 5 reads")
...
if not opts.icm:
    ...
    if ident_calc_method != "ident_cov" and ident_calc_method != "ident_map":
        print(
            "Error with -icm input. Should be ident_cov or ident_map. Run terminated."
        )
...
if sam_file.endswith("bam"):
    ...
    if bam_flag == "SAM":
        print("You designated SAM input but are supplying BAM."
              "Please use the same format for input as specified.")
        sys.exit()

if sam_file.endswith("sam"):
    input_sambam_flag = "SAM"
    if bam_flag == "BAM":
        print("You designated BAM input but are supplying SAM."
              "Please use the same format for input as specified.")
        sys.exit()

if input_sambam_flag == "na":
    print(
        "Input SAM/BAM file not recoginized from extension format designation."
    )
...
# trans_report_line = "\t".join([
#     "transcript_id","num_clusters","high_coverage",
#     "low_coverage","high_quality_percent","low_quality_percent",
#     "start_wobble_list","end_wobble_list","collapse_sj_start_err",
#     "collapse_sj_end_err","collapse_error_nuc","sj_error_simple"
# ])
...
# trans_clust_line = "\t".join([
#     "transcript_id","cluster_id","scaffold",
#     "strand","start","end",
...
# rtswitch_file_line = "\t".join([
#     "trans_id","junct_num","first_seq",
#     "second_seq","rev_comp_first_seq"
# ])
...
lde_file_line = "\t".join([
    ...
    "num_exons",
    "bad_sj_num_line",
    "bad_sj_error_count_line",
    "sj_error_profile_idmsh",
    "sj_error_nuc",
    "sj_error_simple",
    "cigar",
])
...
variation_dict = (
    {}
)  # variation_dict[scaffold][position][variant type][alt allele][cluster id] = 1
...
def track_time(start_time, prev_time):
    ...
    print("time taken since last check:\t" + str(tt_hours) + ":" +
          str(tt_minutes) + ":" + str(tt_seconds))
    ...
    tt_seconds = leftover_time
    print("time taken since beginning:\t" + str(tt_hours) + ":" +
          str(tt_minutes) + ":" + str(tt_seconds))
...
def calc_error_rate(start_pos, cigar, seq_list, scaffold, read_id):
    ...
    all_nuc_mismatch_list = (
        [])  # this shows the nuc mismatch for the all_genome_mismatch_list
    ...
    for i in range(len(cig_dig_list)):
        ...
        if cig_flag == "H":
            ...
            var_seq = "N" * h_count
            var_type = cig_flag
            update_variation_dict(scaffold, var_pos, var_type, var_seq,
                                  read_id)
            ...
            genome_mismatch_list, seq_mismatch_list, nuc_mismatch_list = mismatch_seq(
                genome_slice, seq_slice, genome_start, seq_start)
            ...
            var_pos = genome_pos
            var_seq = str(
                deletion_length)  # var seq for deletion is length of deletion
            var_type = cig_flag
            update_variation_dict(scaffold, var_pos, var_type, var_seq,
                                  read_id)
            # for variation detection end
            ...
            sj_pre_error_builder_list = (
                [])  # list of cigar string before splice junctions
            sj_post_error_builder_list = (
                [])  # list of cigar string after splice junctions
            ...
            while prev_total_mismatch_length <= sj_err_threshold and prev_sj_flag == 0:
                ...
                if prev_cig_flag == "M":
                    # if no mismatch
                    # no mismatch from M cigar regions up to this point
                    if len(all_genome_mismatch_list) < 1:
                        # last_genome_mismatch = -2 * sj_err_threshold

                        # no mis matches for this gene
                        if prev_total_mismatch_length <= sj_err_threshold:
                            sj_pre_error_builder_list.append(
                                str(prev_cig_length) + prev_cig_flag)

                    elif all_genome_mismatch_index >= 0:
                        last_genome_mismatch = all_genome_mismatch_list[
                            all_genome_mismatch_index]
                        last_nuc_mismatch = all_nuc_mismatch_list[
                            all_genome_mismatch_index]
                        ...
                        # while dist_prev_mismatch <= this_mismatch_length and
                        # dist_prev_mismatch <= sj_err_threshold:
                        while (
                                last_genome_mismatch
                                >= this_cig_genome_pos - prev_cig_length
                                and last_genome_mismatch <= this_cig_genome_pos
                                and dist_prev_mismatch <= sj_err_threshold
                        ):  # dist_prev_mismatch is still in range of sj threshold

                            sj_pre_error_builder_list.append(
                                str(dist_prev_mismatch) + "." +
                                last_nuc_mismatch)
                        ...
                        if m_builder_add_count == 0:
                            # still within in sj threshold so output
                            if prev_total_mismatch_length <= sj_err_threshold:
                                sj_pre_error_builder_list.append(
                                    str(prev_cig_length) + prev_cig_flag)

                    elif (
                            all_genome_mismatch_index < 0
                    ):  # length of mismatch list is not 0 but index has gone past that
                        # last_genome_mismatch = -2 * sj_err_threshold
                        # still within in sj threshold so output
                        if prev_total_mismatch_length <= sj_err_threshold:
                            sj_pre_error_builder_list.append(
            ...
            if len(sj_pre_error_builder_list
                   ) == 0:  # in case no errors are found
            ...
            #########################################
            # not related to sj error should probably move
            # this up or down code of this block
            #########################################
            ...
            while next_total_mismatch_length <= sj_err_threshold and next_sj_flag == 0:
                ...
                if next_cig_flag == "M":
                    ...
                    seq_slice = seq_list[seq_start:seq_end]
                    genome_slice = fasta_dict[scaffold][
                    ...
                    genome_mismatch_list, seq_mismatch_list, nuc_mismatch_list = (
                        mismatch_seq(genome_slice, seq_slice, genome_start,
                                     seq_start))

                    genome_mismatch_index = 0

                    # if no mismatch
                    if len(genome_mismatch_list
                           ) < 1:  # no mismatch in this cig entry
                        # dist_next_mismatch = 2 * sj_err_threshold

                        if next_total_mismatch_length <= sj_err_threshold:

                            sj_post_error_builder_list.append(
                                str(next_cig_length) + next_cig_flag)

                    elif genome_mismatch_index <= len(genome_mismatch_list):
                        nuc_mismatch_list[0]
                        # dist_next_mismatch = genome_mismatch_list[0] - this_genome_pos
                        dist_next_mismatch = genome_mismatch_list[
                            0] - genome_pos

                        m_builder_add_count = 0

                        while (dist_next_mismatch <= this_mismatch_length
                               and dist_next_mismatch < sj_err_threshold):
                            next_genome_mismatch = genome_mismatch_list[
                                genome_mismatch_index]
                            next_nuc_mismatch = nuc_mismatch_list[
                                genome_mismatch_index]

                            # dist_next_mismatch = next_genome_mismatch -
                            # this_genome_pos
                            dist_next_mismatch = next_genome_mismatch - genome_pos

                            # this mismatch goes beyond this M length
                            if dist_next_mismatch <= sj_err_threshold:

                                sj_post_error_builder_list.append(
                                    str(dist_next_mismatch) + "." +
                                    next_nuc_mismatch)

                                m_builder_add_count += 1

                            genome_mismatch_index += 1
                            if genome_mismatch_index >= len(
                                    genome_mismatch_list):
                                break
            ...
            if len(sj_post_error_builder_list
                   ) == 0:  # in case no errors are found
            ...
            sj_pre_error_builder_list_reverse = list(
                reversed(sj_pre_error_builder_list))
            sj_pre_error_builder_line = "_".join(
                sj_pre_error_builder_list_reverse)
...
class Transcript:
    ...
    def add_sam_info(self, sam_flag, scaff_name, start_pos, cigar, read_seq,
                     seq_list):
    ...
    def make_sj_hash_string(self):
        ...
        for exon_index in range(len(self.exon_start_list)):
            exon_start_string_list.append(str(
                self.exon_start_list[exon_index]))
    ...
    def make_sj_hash_int(self):
        ...
        for exon_index in range(len(self.exon_start_list) - 1):
            self.sj_hash = self.sj_hash + (self.exon_start_list[exon_index + 1]
                                           * self.exon_end_list[exon_index])
    ...
    def calc_coverage(self):
        ...
        percent_cov = float(self.seq_length - self.h_count -
                            self.s_count) / float(self.seq_length)
    ...
    def calc_identity(self):
        ...
        if ident_calc_method == "ident_cov":
            ...
            nonmatch_count = (self.h_count + self.s_count + self.i_count +
                              self.d_count + self.mis_count)
            percent_identity = float(self.seq_length - nonmatch_count) / float(
                self.seq_length)
            ...
            percent_identity = float(map_seq_length -
                                     nonmatch_count) / float(map_seq_length)
            percent_identity = percent_identity * 100.0
    ...
    def add_polya_info(self, downstream_seq, dseq_length, a_count, n_count,
                       a_percent, n_percent):
...
class Merged:
    ...
    def format_trans_report_line(self):
        ...
        for i in range(len(self.collapse_sj_start_err_list)):
            collapse_sj_start_err_list_str.append(
                str(self.collapse_sj_start_err_list[i]))
            collapse_sj_end_err_list_str.append(
                str(self.collapse_sj_end_err_list[i]))

        if self.strand == "+":
            collapse_sj_start_err_list_str = list(
                reversed(collapse_sj_start_err_list_str))
            collapse_sj_end_err_list_str = list(
                reversed(collapse_sj_end_err_list_str))

        collapse_sj_start_err_list_line = ",".join(
            collapse_sj_start_err_list_str)
        ...
        if len(collapse_error_nuc_list) > 1:
            ...
            if self.strand == "+":
                if collapse_error_nuc_list[-1] == "na":
                    ...
                    collapse_error_nuc_list = list(
                        reversed(collapse_error_nuc_list))
                else:
        ...
        #
        #                sj_pre_error_simple_string,
        #                sj_post_error_simple_string = simple_sj_error(
        #                    sj_pre_error_split, sj_post_error_split
        # .               )
        #
        #                sj_both_error_simple_string = sj_pre_error_simple_string +
        #                    ">" + sj_post_error_simple_string
        #
        #                all_sj_both_error_simple_list.append(sj_both_error_simple_string)
        #
        #            all_sj_both_error_simple_list_line = ";".join(
        #                all_sj_both_error_simple_list
        #            )
        #        else:
...
def fuzzy_match(coord1, coord2, diff_threshold
                ):  # use this to allow for fuzzy matches of splice junctions
    diff_num = 0
    match_flag = "none"
    # diff_threshold = 10
...
def exact_match_capped(trans_obj, o_trans_obj, strand):
    ...
    if strand == "+":
        if (this_start_diff <= fiveprime_threshold
                and this_end_diff <= threeprime_threshold):
        ...
        if (this_start_diff <= threeprime_threshold
                and this_end_diff <= fiveprime_threshold):
...
def compare_transcripts(trans_obj, o_trans_obj, fiveprime_cap_flag,
                        strand):  # use this to compare two transcripts
    ...
    if (fiveprime_cap_flag == "capped" and diff_num_exon_flag
            != 0):  # if 5prime capped then should have same number of exons
        ...
        if len(e_start_list) > len(o_e_start_list):
            ...
            if strand == "+":
                ...
                if e_end_list[0] > o_e_end_list[0]:
                    ...
                    print("Error with short and long trans identification")
                    print(trans_obj.cluster_id + " - " +
                          o_trans_obj.cluster_id + " strand: " + strand)
                    sys.exit()
        ...
        for i in range(min_exon_num):
            ...
            if strand == "+":
                # iterate from last exon to account for possible 5' degradation for
                # forward strand
                j = -1 * (i + 1)
            elif strand == "-":
            ...
            if strand == "+":
                ...
                if (diff_num_exon_flag == 0 and i == min_exon_num -
                        1):  # use 5 prime threshold if this is first exon
            ...
            start_match_flag, start_diff_num = fuzzy_match(
                e_start, o_e_start, start_threshold)
            end_match_flag, end_diff_num = fuzzy_match(e_end, o_e_end,
                                                       end_threshold)

            # use this for the condition that no 5 prime cap and shorter 5 prime end
            if (fiveprime_cap_flag == "no_cap" and i == min_exon_num -
                    1):  # if no 5 cap and this is last 5 prime exon
                if strand == "+":
                    if end_match_flag == "no_match":
                        ...
                        if (b_trans_id == short_trans and start_diff_num < 0
                                and diff_num_exon_flag != 0):
                            trans_comp_flag = "same_three_prime_diff_exons"
                        elif (a_trans_id == short_trans and start_diff_num > 0
                              and diff_num_exon_flag != 0):
                            trans_comp_flag = "same_three_prime_diff_exons"
                        # elif short_trans == "same" and start_diff_num < 0 and
                        # diff_num_exon_flag == 0: # if short trans is same then same
                        # number of exons
                        elif (
                                start_diff_num < 0 and diff_num_exon_flag == 0
                        ):  # if short trans is same then same number of exons
                        ...
                        # number of exons
                            ...
                            all_match_flag = 0
                        ################################################################
                        # Continue here! 2017-06-13
                        # Figure out how to differentiate shorter transcript!!!!!
                        # Need to define shorter/longer roles for trans grouping!!
                        ################################################################
                elif strand == "-":
                    if start_match_flag == "no_match":
                        ...
                        if (b_trans_id == short_trans and end_diff_num > 0
                            ...
                            trans_comp_flag = "same_three_prime_diff_exons"
                              ...
                              and diff_num_exon_flag != 0):
                            trans_comp_flag = "same_three_prime_diff_exons"
                        # elif short_trans == "same" and end_diff_num > 0 and
                        # diff_num_exon_flag == 0: # if short trans is same then same
                        # number of exons
                        elif (
                                end_diff_num > 0 and diff_num_exon_flag == 0
                        ):  # if short trans is same then same number of exons
                            short_trans = b_trans_id
                            trans_comp_flag = "same_three_prime_same_exons"
                        # elif short_trans == "same" and end_diff_num < 0 and
            ...
            # if capped or if nocap without being the last exon use normal matching
            # procedure
            else:
                if start_match_flag == "no_match":
                    all_match_flag = 0
    ...
    #        if(trans_obj.trans_id == '1_14_c81301/2/1370' and
    #        o_trans_obj.trans_id == '1_14_c147778/1/1455'):
    #            print("blah blah")
    #            print(trans_comp_flag)
    #            print(start_match_list)
    #            print(end_match_list)
    ...
    #            print(min_exon_num)
    #            sys.exit()
    #        if(trans_obj.trans_id == '1_14_c147778/1/1455' and
    #        o_trans_obj.trans_id == '1_14_c81301/2/1370'):
    #            print("blah blah")
    #            print(trans_comp_flag)
    #           print(start_match_list)
    #            print(end_match_list)
...
def sj_error_priority_finder(trans_obj, i, max_exon_num):
    ...
    if len(e_start_list) > 1:  # if there are splice junctions to cover
        ...
        if strand == "+":
            if i == 0:  # last 3' exon accoridng to genome
                ...
                e_start_priority = sj_error_priority_start(
                    sj_pre_error, sj_post_error)

                e_start_priority_error = (sj_pre_error +
                                          priority_error_delimit +
                                          sj_post_error)
                ...
                e_start_priority = sj_error_priority_start(
                    sj_pre_error_start, sj_post_error_start)
                e_end_priority = sj_error_priority_end(sj_pre_error_end,
                                                       sj_post_error_end)

                e_start_priority_error = (sj_pre_error_start +
                                          priority_error_delimit +
                                          sj_post_error_start)
                e_end_priority_error = (sj_pre_error_end +
                                        priority_error_delimit +
                                        sj_post_error_end)
            ...
            if i == 0:  # 5' first exon according to genome
                ...
                e_end_priority = sj_error_priority_end(sj_pre_error,
                                                       sj_post_error)

                e_end_priority_error = (sj_pre_error + priority_error_delimit +
                                        sj_post_error)
    ...
    if sj_priority_flag == "no_priority":
        ...
        print("""
            Error with splice junction priority flag.
            Please use no_priority or sj_priority.
            """)
        sys.exit()
    ...
    return (
        e_start_priority,
        e_end_priority,
        e_start_priority_error,
        e_end_priority_error,
    )
...
def length_error_type(error_string, error_report):
    ...
    if error_type == "I":
        ...
        error_length = int(error_string.split("H")[0])
          ...
          ):  # this is a mismatch error representing only one nt position
        error_length = 1
    ...
    for sj_post_error_string in sj_post_error_split:
        ...
        if len(sj_post_error_string.split(".")) == 3:
            ...
            if sj_post_error_string == "0":
                ...
                [cig_dig_list,
                 cig_char_list] = cigar_list(sj_post_error_string)
...
def sj_error_local_density(trans_obj):
    ...
    bad_sj_num_list = (
        []
    )  # list of the splice junctions that failed to pass quality threshold
    ...
    for i in range(max_exon_num - 1):
        ...
        sj_pre_error_simple_string, sj_post_error_simple_string = simple_sj_error(
            sj_pre_error_split, sj_post_error_split)
        ...
        all_sj_both_error_simple_list.append(sj_pre_error_simple_string + ">" +
                                             sj_post_error_simple_string)
        ...
        for sj_pre_error_char in sj_pre_error_split:
            ...
            error_length, error_type = length_error_type(
                sj_pre_error_char, trans_cigar)
            ##########################################################################
            ##########################################################################
        ...
        for sj_post_error_char in sj_post_error_split:
            ...
            error_length, error_type = length_error_type(
                sj_post_error_char, trans_cigar)
        ...
        sj_pre_error_all_count = (sj_pre_error_i_count + sj_pre_error_d_count +
                                  sj_pre_error_m_count + sj_pre_error_s_count +
                                  sj_pre_error_h_count)
        sj_post_error_all_count = (sj_post_error_i_count +
                                   sj_post_error_d_count +
                                   sj_post_error_m_count +
                                   sj_post_error_s_count +
                                   sj_post_error_h_count)

        sj_pre_error_all_line = ",".join([
            str(sj_pre_error_i_count),
            str(sj_pre_error_d_count),
            str(sj_pre_error_m_count),
            str(sj_pre_error_s_count),
            str(sj_pre_error_h_count),
        ])
        sj_post_error_all_line = ",".join([
            str(sj_post_error_i_count),
            str(sj_post_error_d_count),
            str(sj_post_error_m_count),
            str(sj_post_error_s_count),
            str(sj_post_error_h_count),
        ])

        sj_all_error_all_line = ">".join(
            [sj_pre_error_all_line, sj_post_error_all_line])
        ...
        sj_error_string = (str(sj_pre_error_all_count) + ">" +
                           str(sj_post_error_all_count))
...
def collapse_transcripts(trans_obj_list, fiveprime_cap_flag,
                         collapse_flag):  # use this to collapse transcripts
    ...
    for i in range(max_exon_num):  # go from 3 prime end
        ...
        e_start_dict = (
            {})  # e_start_dict[priority number][start] = number of occurrences
        e_end_dict = {
        }  # e_end_dict[priority number][end] = number of occurrences

        priority_error_start_dict = (
            {}
        )  # priority_error_start_dict[priority number][start][error string] = 1
        priority_error_end_dict = (
            {}
        )  # priority_error_end_dict[priority number][end][error string] = 1
        ...
        for trans_obj in trans_obj_list:
            ...
            # figure out errors near splice junctions
            ######################################################
            trans_obj.sj_pre_error_list
            trans_obj.sj_post_error_list

            this_max_exon_num = len(e_start_list)

            if i >= len(e_start_list
                        ):  # use for no cap when exon numbers may not match
                continue
            ...
            (
                ...
                e_start_priority_error,
                e_end_priority_error,
            ) = sj_error_priority_finder(trans_obj, i, this_max_exon_num)
        ...
        if num_most_starts > 1:
            ...
            if num_trans > 2:
                if log_flag == "log_on":
                    print("more than one best e start! " + str(best_e_start) +
                          " num_trans: " + str(num_trans))
        ##########################################
        ...
        if num_most_ends > 1:
            ...
            if num_trans > 2:
                if log_flag == "log_on":
                    print("more than one best e end! " + str(best_e_end) +
                          " num_trans: " + str(num_trans))
        ##########################################
        ...
        if (fiveprime_cap_flag == "no_cap" and i + 1
                == max_exon_num):  # use earliest 5' end for no cap libraries
            if strand == "+":
                best_e_start = long_e_start
            elif strand == "-":
                best_e_end = long_e_end

        # allow user to choose whether to use the most common ends or the longest
        # ends, default is most common ends
        if collapse_flag == "longest_ends":
            if i + 1 == max_exon_num:
                if strand == "+":
                    best_e_start = long_e_start
                elif strand == "-":
        ...
        ##########################################################
        priority_error_start_list = list(
            priority_error_start_dict[best_start_priority]
            [best_e_start].keys())
        priority_error_end_list = list(
            priority_error_end_dict[best_end_priority][best_e_end].keys())
    ...
    collapse_start_list, start_wobble_list = zip(
        *sorted(zip(collapse_start_list, start_wobble_list)))
    collapse_end_list, end_wobble_list = zip(
        *sorted(zip(collapse_end_list, end_wobble_list)))
    ...
    for i in range(len(collapse_start_list)):
        ...
        # next start should always be later than prev start
        if check_start <= prev_start:
    ...
    # flip order of sj err list if the strand is positive
    # since we started from the 3' end.
    #    if strand == "+":
...
def gene_group(trans_list,
               ):  # groups trans into genes, does not take into account strand
    ...
    for i in range(len(trans_obj_list)):
        ...
        for j in range(i + 1, len(trans_obj_list)):
            ...
            if (trans_id in trans_gene_dict and o_trans_id
                    in trans_gene_dict):  # skip if already in same group
            ...
            if overlap_flag == 1:
                # if no gene groups make new one
                if (trans_id not in trans_gene_dict
                        and o_trans_id not in trans_gene_dict):
                    gene_count += 1
                    trans_gene_dict[trans_id] = gene_count
                    trans_gene_dict[o_trans_id] = gene_count
                    gene_trans_dict[gene_count] = {}
                    ...
                    if gene_num != o_gene_num:  # merge gene groups
                        m_trans_id_list = list(
                            gene_trans_dict[o_gene_num].keys())
    ...
    gene_start_trans_dict = {
    }  # gene_start_trans_dict[gene start][trans id] = 1
    ...
    while sort_flag == 0:
        ...
        if len(same_order_index_list) == 0:
            ...
            for pos_element in same_order_index_list:
                slice_pos_index_list = list(
                    same_order_index_dict[pos_element].keys())
                ...
                for this_pos in slice_pos_index_list:
                    if past_pos == "na":
                        ...
                        print("""
                            Error with non-consequtive indices for
                            same pos in trans sorting.
                            """)
                ...
                [list_slice,
                 sort_flag] = iterate_sort_list(list_slice, pos_index + 1)
...
def sort_pos_trans_list(pos_trans_list, pos_trans_dict):
    ...
    [list_trans_pos_list,
     sort_flag] = iterate_sort_list(list_trans_pos_list, pos_index)
...
def sort_transcripts(trans_obj_list):
    ...
    for trans_obj in trans_obj_list:
        ...
        if trans_pos_line in pos_trans_dict:
            ...
            pos_trans_dict[trans_pos_line].trans_id
            trans_obj.trans_id
            ...
            if log_flag == "log_on":
                ...
                print(
                    list(pos_trans_dict[trans_pos_line].merged_trans_dict.keys(
                    )))
            ...
            for a_uniq_trans_id in trans_obj.merged_trans_dict:
                a_bed_line = trans_obj_dict[a_uniq_trans_id].format_bed_line(
                    a_uniq_trans_id)
                if log_flag == "log_on":
                    print(a_bed_line)
            if log_flag == "log_on":
                print("b###########################################")
            for b_uniq_trans_id in pos_trans_dict[
                    trans_pos_line].merged_trans_dict:
                b_bed_line = trans_obj_dict[b_uniq_trans_id].format_bed_line(
                    b_uniq_trans_id)
                if log_flag == "log_on":
                    print(b_bed_line)
            if log_flag == "log_on":
            ...
            if duplicate_flag == "no_merge":
                print("""
                    By default TAMA collapse does not allow merging of
                    duplicate transcript groups.
                    """)
                print("""
                    Duplicate transcript groups occur when different groupings of
                    transcripts results in the same collapsed model.
                    """)
                print("""
                    If you would like to merge duplicate transcript groups
                    please add -d merge_dup to the arguments.
                    """)
                sys.exit()
                ...
                for a_uniq_trans_id in trans_obj.merged_trans_dict:
                    ...
                    pos_trans_dict[trans_pos_line].add_merged_trans(
                        trans_obj.merged_trans_dict[a_uniq_trans_id])
                ...
                for b_uniq_trans_id in pos_trans_dict[
                        trans_pos_line].merged_trans_dict:
                    match_trans_obj_list.append(
                        pos_trans_dict[trans_pos_line].
                        merged_trans_dict[b_uniq_trans_id])
                ...
                # end_wobble_list,
                # e_start_trans_dict,
                # e_end_trans_dict = collapse_transcripts(
                #     match_trans_obj_list,
                #     collapse_flag
                # )
                (
                    ...
                    collapse_start_error_nuc_list,
                    collapse_end_error_nuc_list,
                ) = collapse_transcripts(match_trans_obj_list,
                                         fiveprime_cap_flag, collapse_flag)

                # update merge info
                # pos_trans_dict[trans_pos_line].add_merge_info(
                #     collapse_start_list,
                #     collapse_end_list,
                #     start_wobble_list,
                #     end_wobble_list,
    ...
    [new_pos_trans_list,
     new_pos_trans_dict] = sort_pos_trans_list(pos_trans_list, pos_trans_dict)
...
def longest_transcript(trans_id, trans_id_list):
    ...
    longest_num_exons = 0
    longest_tss_pos = -1
    # longest_trans_id_dict = {}
    # longest_trans_id_dict[trans id] = longest_tss_pos
    ...
    if strand == "+":
        if num_exons > longest_num_exons:
            ...
            if trans_start < longest_tss_pos:
                ...
                start_match_flag, start_diff_num = fuzzy_match(
                    trans_start, longest_tss_pos, exon_diff_threshold)
                ...
                print("""'
                    Error with trans_start/longest_tss_pos comparison
                    in longest_transcript
                    """)
                sys.exit()
        elif num_exons < longest_num_exons:
            longest_trans = "short"
        ...
        if num_exons > longest_num_exons:
            ...
            if trans_end > longest_tss_pos:
                ...
                start_match_flag, start_diff_num = fuzzy_match(
                    trans_end, longest_tss_pos, exon_diff_threshold)
                if start_match_flag == "wobbly_match":
                    longest_trans = "long"
                elif start_match_flag == "no_match":
                    longest_trans = "short"
...
class TransGroup:
    ...
    def __init__(self, group_name):
        ...
        self.trans_group_dict = (
            {}
        )  # trans_group_dict[trans id][trans group] = "longest" or "short" for no cap
        self.group_trans_dict = (
            {}
        )  # group_trans_dict[group num][trans id] = "longest" or "short" for no cap
        self.group_count = 0
        self.group_longest_dict = (
            {})  # group_longest_dict[group num][longest/long][trans] = 1

        self.group_max_exon_dict = {
        }  # group_max_exon_dict[group num] = max exon num
    ...
    def delete_trans(self, trans_a):
        ...
        for a_trans_group in self.trans_group_dict[trans_a]:
            ...
            # if ther group is now empty, delete it
            if len(list(self.group_trans_dict[a_trans_group])) == 0:
    ...
    def new_group_a(self, trans_a):
        ...
        if log_flag == "log_on":
            print("invoke new_group_a " + str(self.group_count) +
                  " newgroup " + trans_a)
        self.group_count += 1
        if trans_a in self.trans_group_dict:
        ...
        self.group_max_exon_dict[
            self.group_count] = trans_obj_dict[trans_a].num_exons
    ...
    def add_a_to_b_group(self, trans_a, trans_b):
        ...
        trans_obj_dict[trans_a]
        trans_obj_dict[trans_b]
        ...
        if len(list(self.trans_group_dict[trans_a].keys())
               ) == 1:  # if only one group
            a_trans_group = list(self.trans_group_dict[trans_a].keys())[0]
            if log_flag == "log_on":
                print(trans_a)
            # if only this in one group
            if len(list(self.group_trans_dict[a_trans_group].keys())) == 1:
                if list(self.group_trans_dict[a_trans_group].keys()
                        )[0] == trans_a:
                    self.group_trans_dict.pop(a_trans_group, None)
                    self.trans_group_dict.pop(trans_a, None)
        ...
        if trans_a not in self.trans_group_dict:
            ...
            # this happens if trans_a is a nocap trans in which case it can be in
            # multiple groups
            if log_flag == "log_on":
                print("trans_a already in group, should be nocap trans: " +
                      trans_a)
        ...
        for b_group_num in list(self.trans_group_dict[trans_b].keys()):
            ...
            # if a is longest in any of it's groups then
            # you can add other trans from those groups to b
            ...
            # dont need to redo longest and short because added a trans is all short
            # compared to b group
    ...
    def merge_a_b_groups(self, trans_a, trans_b):
        if log_flag == "log_on":
            print("invoke merge_a_b_groups " + str(self.group_count) + " " +
                  trans_a + " " + trans_b)
        ...
        # find bigger group
        num_trans_group_a = len(list(
            self.group_trans_dict[a_group_num].keys()))
        num_trans_group_b = len(list(
            self.group_trans_dict[b_group_num].keys()))

        merge_group_num = -1
        if num_trans_group_a > num_trans_group_b:
            merge_group_num = a_group_num
            self.group_max_exon_dict[
                merge_group_num] = self.group_max_exon_dict[a_group_num]
            ...
            self.group_max_exon_dict[
                merge_group_num] = self.group_max_exon_dict[b_group_num]
    ...
    def merge_a_b_groups_nocap(self, trans_a, trans_b):
        ...
        if log_flag == "log_on":
            print("invoke merge_a_b_groups_nocap " + str(self.group_count) +
                  " " + trans_a + " " + trans_b)
        # only cap lib trans should be used for merging groups
        if len(list(self.trans_group_dict[trans_a].keys())) > 1:
            if log_flag == "log_on":
        ...
        if trans_obj_a.num_exons != trans_obj_b.num_exons:
            ...
            print(trans_a + " " + trans_b)
            print(
                str(trans_obj_a.num_exons) + " " + str(trans_obj_b.num_exons))
            sys.exit()
        ...
        for a_group_num in a_group_num_list:
            # if this trans has as many exons as longest trans in group
            if (self.group_max_exon_dict[a_group_num] ==
                    trans_obj_dict[trans_a].num_exons):
                # if self.group_trans_dict[a_group_num][trans_a].startswith("long"): #
                # could be longest or long
                for group_trans in self.group_trans_dict[a_group_num]:
                    self.group_trans_dict[
                        self.group_count][group_trans] = 1  # make new group
                    self.trans_group_dict[group_trans].pop(
                        a_group_num, None)  # remove old group num
                    self.trans_group_dict[group_trans][
                        self.group_count] = 1  # add new group num

                self.group_max_exon_dict[
                    self.group_count] = self.group_max_exon_dict[a_group_num]
                # remove old group
                self.group_trans_dict.pop(a_group_num, None)
                self.group_longest_dict.pop(a_group_num, None)
        ...
        for b_group_num in b_group_num_list:
            # if this trans has as many exons as longest trans in group
            if (self.group_max_exon_dict[b_group_num] ==
                    trans_obj_dict[trans_b].num_exons):
                # if self.group_trans_dict[b_group_num][trans_b].startswith("long"): #
                # could be longest or long
                for group_trans in self.group_trans_dict[b_group_num]:
                    self.group_trans_dict[
                        self.group_count][group_trans] = 1  # make new group
                    self.trans_group_dict[group_trans].pop(
                        b_group_num, None)  # remove old group num
                    self.trans_group_dict[group_trans][
                        self.group_count] = 1  # add new group num

                if (self.group_max_exon_dict[self.group_count]
                        < self.group_max_exon_dict[b_group_num]):
                    self.group_max_exon_dict[self.group_count] = (
                        self.group_max_exon_dict[b_group_num])
                # remove old group
                self.group_trans_dict.pop(b_group_num, None)
        ...
        if len(trans_id_list) > 0:  # new group was made
            ...
            for check_trans_id in trans_id_list:
                longest_trans_flag = longest_transcript(
                    check_trans_id, [longest_trans_id])
            ...
            for trans_c in trans_id_list:
                longest_trans_flag = longest_transcript(
                    trans_c, [longest_trans_id])
                self.trans_group_dict[trans_c][
                    self.group_count] = longest_trans_flag
                self.group_trans_dict[
                    self.group_count][trans_c] = longest_trans_flag

                if longest_trans_flag == "longest":
                    self.group_longest_dict[
                        self.group_count]["longest"][trans_c] = 1
...
def simplify_gene_capped(
    trans_obj_list, fiveprime_cap_flag
):  # goes through transcripts in gene and groups transcripts for collapsing
    # for capped only!!
    if log_flag == "log_on":
    ...
    # ungrouped_trans_obj_dict[cluster_id] =  trans_obj
    # clusters that have been grouped
    # grouped_trans_obj_dict = {}
    # grouped_trans_obj_dict[cluster_id] =  trans_obj
    # grouped clusters that have been not been searched/been used as a hunter
    unsearched_trans_obj_dict = {
    }  # unsearched_trans_obj_dict[cluster_id] =  trans_obj
    ...
    while ungrouped_count > 0:
        ...
        while unsearched_count > 0 and ungrouped_count > 0:
            ...
            if hunter_cluster_id == "new_hunter":
                unsearched_cluster_list = list(
                    unsearched_trans_obj_dict.keys())
            ...
            for prey_cluster_id in ungrouped_trans_obj_dict:
                ...
                prey_trans_obj.num_exons

                # this condition should not happen anymore because i delete hunters
                # after I use them
                ...
                group_match_flag = transgroup.check_same_group(
                    hunter_cluster_id, prey_cluster_id)
                ...
                (
                    ...
                    min_exon_num,
                    diff_num_exon_flag,
                ) = compare_transcripts(hunter_trans_obj, prey_trans_obj,
                                        fiveprime_cap_flag, hunter_strand)

                # added to speed things up

                # if num_group_reads < sj_hash_read_threshold or hunter_num_exons < 3:
                #    trans_comp_flag, start_match_list, start_diff_list,
                #    end_match_list, end_diff_list, short_trans,
                #    long_trans, min_exon_num, diff_num_exon_flag = compare_transcripts(
                #        hunter_trans_obj, prey_trans_obj,
                #        fiveprime_cap_flag, hunter_strand
                #    )
                #
                # else:
                #
                #    exact_match_flag = exact_match_capped(
                #        hunter_trans_obj,
                #        prey_trans_obj,
                #        hunter_strand
                #    )
                ...
                #        trans_comp_flag = "same_transcript"
                #
                #    # exact match is the same as compare for single exon transcripts
                ...
                #        trans_comp_flag, start_match_list, start_diff_list,
                #       end_match_list, end_diff_list,  short_trans,
                #       long_trans, min_exon_num,
                #       diff_num_exon_flag = compare_transcripts(
                #            hunter_trans_obj, prey_trans_obj,
                #            fiveprime_cap_flag, hunter_strand
                #        )
                #
                ...
                # old system used to generalize for nocap mode
                # but this slows down capped mode
                # trans_match_flag = 0
                ...
                # if they are in different groups, merge groups, applies to both caps
                else:
                    transgroup.merge_a_b_groups(hunter_cluster_id,
                                                prey_cluster_id)

                    # remove the prey cluster from dict to avoid redundant searching
                    # remove this outside of for loop
                    # ungrouped_trans_obj_dict.pop(prey_cluster_id)
...
def simplify_gene_nocap(
    ...
    exon_trans_obj_dict = {
    }  # exon_trans_obj_dict[num exons][cluster id] = trans_obj
    # use this to refresh sub exon dicts
    sub_exon_cluster_dict = {
    }  # sub_exon_cluster_dict[num exons][cluster id] = 1
    ...
    sub_length_cluster_dict = (
        {}
    )  # sub_length_cluster_dict[cluster id] = 1  use this to mark degraded clusters
    ...
    while exon_num_index < len(exon_num_list):
        ...
        for ungroup_cluster_id in exon_trans_obj_dict[exon_num_level]:
            ungrouped_trans_obj_dict[ungroup_cluster_id] = exon_trans_obj_dict[
                exon_num_level][ungroup_cluster_id]
        ...
        while ungrouped_count > 0:
            ...
            if unsearched_count == 0:
                ...
                ungrouped_strand = ungrouped_trans_obj_dict[
                    ungrouped_cluster_list[0]].strand
                # sort by 5' end length to deal with issue of long based nocap merging
                ungrouped_cluster_sort_dict = (
                    {})  # ungrouped_cluster_sort_dict[start][cluster id] = 1
                for ungrouped_cluster_id in ungrouped_cluster_list:
                    if ungrouped_strand == "+":
                        ungrouped_five_prime = ungrouped_trans_obj_dict[
                            ungrouped_cluster_id].start_pos
                    elif ungrouped_strand == "-":
                        ungrouped_five_prime = ungrouped_trans_obj_dict[
                            ungrouped_cluster_id].end_pos
                    else:
                    ...
                    ungrouped_cluster_sort_dict[ungrouped_five_prime][
                        ungrouped_cluster_id] = 1

                ungrouped_cluster_five_list = list(
                    ungrouped_cluster_sort_dict.keys())
                if ungrouped_strand == "+":
                ...
                for ungrouped_five_coord in ungrouped_cluster_five_list:
                    for ungrouped_cluster_id in list(
                            ungrouped_cluster_sort_dict[ungrouped_five_coord].
                            keys()):
                        ungrouped_cluster_list_sorted.append(
            ...
            # use this to keep track of sub clusters that have not already been
            ...
            while unsearched_count > 0:
                ...
                if hunter_cluster_id == "new_hunter":
                    ...
                    # hunter trans and prey trans, hunter used to look for prey
                    hunter_cluster_id = unsearched_cluster_list[0]
                    hunter_trans_obj = unsearched_trans_obj_dict[
                        hunter_cluster_id]
                ...
                a_group_check = transgroup.check_trans_status(
                    hunter_cluster_id)
                # make groups for each transcript if no group
                if a_group_check != 1:
                    transgroup.new_group_a(hunter_cluster_id)

                list(ungrouped_trans_obj_dict.keys())
                unsearched_cluster_list = list(
                    unsearched_trans_obj_dict.keys())

                # search at same exon num level
                ########################################################################
                # only search at same level if this has not been grouped with longer
                # transcript
                if hunter_cluster_id not in sub_length_cluster_dict:
                    for prey_cluster_id in ungrouped_trans_obj_dict:

                        prey_trans_obj = ungrouped_trans_obj_dict[
                            prey_cluster_id]

                        prey_strand = prey_trans_obj.strand
                        prey_num_exons = prey_trans_obj.num_exons

                        # this condition should not happen anymore because i delete
                        # hunters after I use them
                        if hunter_cluster_id == prey_cluster_id:
                            continue
                        ...
                        if hunter_strand != prey_strand:
                            print(
                                "Strand of transcripts within gene do not match"
                            )
                            sys.exit()

                        b_group_check = transgroup.check_trans_status(
                            prey_cluster_id)
                        # make groups for each transcript if no group
                        if b_group_check != 1:
                            transgroup.new_group_a(prey_cluster_id)
                        ...
                        # this shoudn't be needed anymore due to new dict based group
                        # search
                        if group_match_flag == 1:  # if they are both in the same group
                            continue
                        ...
                        # if(num_group_reads < sj_hash_read_threshold or
                        # hunter_num_exons < 3):
                        #    trans_comp_flag, start_match_list, start_diff_list,
                        #    end_match_list, end_diff_list, short_trans,
                        #    long_trans, min_exon_num,
                        ...
                        #    )
                        # else:
                        #    exact_match_flag = exact_match_nocap(
                        #        hunter_trans_obj,
                        #        prey_trans_obj,
                        #        hunter_strand
                        #    )
                        #
                        #    if exact_match_flag == "exact_match":
                        #        # Use this for long-long nocap comparison
                        #        trans_comp_flag = "same_three_prime_same_exons"
                        #    elif hunter_num_exons == 1:
                        #        trans_comp_flag = 'diff_transcripts'
                        #    elif exact_match_flag == "not_exact":
                        #        trans_comp_flag, start_match_list, start_diff_list,
                        #        end_match_list, end_diff_list, short_trans,
                        #        long_trans, min_exon_num, diff_num_exon_flag =
                        #            compare_transcripts(
                        #                hunter_trans_obj, prey_trans_obj,
                        #                fiveprime_cap_flag, hunter_strand
                        # .          )
                        ...
                        # print(
                        #     "compare_transcripts: "
                        #     + hunter_trans_obj.cluster_id + "\t" +
                        #     prey_trans_obj.cluster_id+"\t" +
                        #     trans_comp_flag + "\t" +
                        #     str(hunter_trans_obj.num_exons)+ "\t" +
                        #     str(prey_trans_obj.num_exons) + "\t" +
                        #     str(diff_num_exon_flag)
                        # )
                        ...
                        # if they are in different groups, but match
                        else:
                            # same number of exons
                            if hunter_num_exons == prey_num_exons:

                                if trans_comp_flag == "same_transcript":  # 2019/06/07
                                    # print(transgroup.group_trans_dict)
                                    transgroup.merge_a_b_groups_nocap(
                                        hunter_cluster_id, prey_cluster_id)
                                elif (trans_comp_flag ==
                                      "same_three_prime_same_exons"
                                      ):  # 2019/06/07
                                    transgroup.add_a_to_b_group(
                                        prey_cluster_id, hunter_cluster_id)
                                else:
                                    print("Error with match flag")
                                    print(trans_comp_flag)
                                    sys.exit()
                                ...
                                #    elif hunter_num_exons > prey_num_exons:
                                #        # add shorter to longer
                                #        transgroup.add_a_to_b_group(
                                #            prey_cluster_id,
                                #            hunter_cluster_id
                                # .       )
                                #    elif prey_num_exons > hunter_num_exons:
                                #        # add shorter to longer
                                #        transgroup.add_a_to_b_group(
                                #            hunter_cluster_id,
                                #            prey_cluster_id
                                ...
                                # remove the prey cluster from dict to avoid
                                # redundant searching
                                # remove this outside of for loop
                                # ungrouped_trans_obj_dict.pop(prey_cluster_id)
                                # add prey to unsearched dict
                                unsearched_trans_obj_dict[prey_cluster_id] = (
                                    prey_trans_obj)
                ...
                while prey_exon_index < len(exon_num_list):
                    ...
                    for subgroup_cluster_id in this_sub_exon_cluster_dict[
                            prey_exon_level]:

                        subgrouped_trans_obj_dict[subgroup_cluster_id] = (
                            exon_trans_obj_dict[prey_exon_level]
                            [subgroup_cluster_id])

                    for prey_cluster_id in subgrouped_trans_obj_dict:

                        prey_trans_obj = subgrouped_trans_obj_dict[
                            prey_cluster_id]
                        ...
                        if trans_comp_flag == "same_transcript":
                            ...
                            # trans_match_flag = 1
                            print(
                                "Error with subgroup seach same_three_prime_same_exons"
                            )
                            sys.exit()
                        elif trans_comp_flag == "same_three_prime_diff_exons":
                            trans_match_flag = 1
                        ...
                        if trans_match_flag != 1:
                            ...
                            if hunter_num_exons == prey_num_exons:
                                ...
                                sys.exit()
                            elif (hunter_num_exons
                                  > prey_num_exons):  # add shorter to longer
                                transgroup.add_a_to_b_group(
                                    prey_cluster_id, hunter_cluster_id)
                                sub_length_cluster_dict[prey_cluster_id] = 1

                                # this_sub_exon_cluster_dict[prey_exon_level].pop(prey_cluster_id)
                                # ######### 2019/06/06

                            elif (prey_num_exons
                                  > hunter_num_exons):  # add shorter to longer
                                # transgroup.add_a_to_b_group(hunter_cluster_id,prey_cluster_id)
                                print(
                                    "Error with subgroup exon match: hunter smaller"
                                )
                                sys.exit()
...
def detect_polya(
    trans_obj, a_window
):  # looks for a stretch of poly A in the genome region 3' of the end of the transcript
    strand = trans_obj.strand
    ...
    if strand == "+":
        ...
        if dseq_length == 0:
            if log_flag == "log_on":
                print("dseq_length == 0")
                print(trans_obj.trans_id)
                print(scaffold + " " + str(trans_obj.start_pos) + " " +
                      str(trans_obj.end_pos) + " " + strand)
            dseq_length = 1
...
def detect_rt_switch(
    trans_obj,
):  # looks for complementary structure in intronic region that might cause rt switch
    ...
    bind_seq_dict = {
    }  # bind_seq_dict[splice junction]['end seq'/'start seq'] = seq
...
# Helper: gzip-aware FASTA parsing
def _seqio_parse_fasta_any(path):
    """
    Return an iterator of SeqIO records, supporting both plain and gzipped FASTA.
    """
    try:
...
fasta_scaffold_list = [
]  # list of fatsa seq names to be compared to SAM file header

prev_time = track_time(start_time, prev_time)
# Create fasta lookup dict
print("going through fasta")
for seq_record in _seqio_parse_fasta_any(fasta_file_name):
...
if bam_flag == "BAM":
    ...
    # 允许通过环境变量指定 samtools 路径（由封装注入），否则回退到 PATH 中的 samtools
    try:
        import os  # noqa: F401

        samtools_path = os.environ.get("SAMTOOLS_BIN", "samtools")
    except Exception:
    ...
    try:
        ...
        proc_ok = getattr(p, "returncode", 0) == 0
    except OSError:
        proc_ok = False

    if not proc_ok:
        # samtools 不可用或执行失败，回退使用 pysam
        try:
            py_out = pysam.view("-h", sam_file, catch_stdout=True)
            if isinstance(py_out, bytes):
                py_text = py_out.decode("utf-8", "replace")
            else:
                py_text = py_out
            sam_file_contents = py_text.rstrip("\n").split("\n")
        except Exception as e:
            raise OSError(
                "Samtools not found or failed, and pysam fallback errored: %s\n"
                % (str(e)))
    else:
        # Python3: 子进程输出为字节流，需解码
        if isinstance(stdout_b, bytes):
            out_text = stdout_b.decode("utf-8", "replace")
        else:
            out_text = stdout_b
        sam_file_contents = out_text.rstrip("\n").split("\n")
...
if run_mode_flag == "original":
    for line in sam_file_contents:
        ...
        [end_pos, exon_start_list,
         exon_end_list] = trans_coordinates(start_pos, cigar)
        ...
        [
            ...
            sj_pre_error_list,
            sj_post_error_list,
        ] = calc_error_rate(start_pos, cigar, seq_list, scaff_name, read_id)

        trans_obj = Transcript(read_id)
        trans_obj.add_sam_info(sam_flag, scaff_name, start_pos, cigar,
                               read_seq, seq_list)
        trans_obj.add_map_seq_length(map_seq_length)
        trans_obj.add_exon_coords(end_pos, exon_start_list, exon_end_list)
        trans_obj.add_mismatch(
            h_count,
            s_count,
            i_count,
            d_count,
        ...
        if (percent_coverage < coverage_threshold
                or percent_identity < identity_threshold):
            accept_flag = "discarded"
            cluster_line = "\t".join([
                read_id,
                mapped_flag,
                accept_flag,
                percent_coverage_str,
            ...
            (
                ...
                bad_sj_error_count_list,
                lde_outline,
            ) = sj_error_local_density(trans_obj)
            ...
            if bad_sj_flag > 0:
                ...
                # lde_file_line = "\t".join(
                #     [
                #         trans_obj.cluster_id,
                #         trans_obj.scaff_name,
                #         str(trans_obj.start_pos),
                #         str(trans_obj.end_pos),
                #         trans_obj.strand,
                #         bad_sj_num_line,
                #         bad_sj_error_count_line,
                #         trans_obj.cigar,
            ...
            downstream_seq, dseq_length, a_count, n_count, a_percent, n_percent = (
                detect_polya(trans_obj, a_window))
            trans_obj.add_polya_info(downstream_seq, dseq_length, a_count,
                                     n_count, a_percent, n_percent)
            ...
            if read_id in trans_obj_dict:
                ...
                best_trans_obj, best_map_id = compare_multimaps(
                    trans_obj_a, trans_obj_b)

                # only re-assign if the new map is better, otherwise old map is aready
                # processed
                if best_map_id == "B":
                    trans_obj_dict[read_id] = best_trans_obj
                    multimap_flag = 1
                else:
                    # if this new map is not going to be used we can skip the rest of
                    # the loop
                    continue
        ...
        if multimap_flag == 1:
            ...
            if len(group_trans_list_dict[old_group_count]) > 1:
                group_trans_list_dict[old_group_count].remove(
                    read_id)  # remove read from old group
                trans_group_dict.pop(
                    read_id, None
                )  # remove read from trans_group_dict, will be re-assigned later

            elif (len(group_trans_list_dict[old_group_count]) == 1
                  ):  # group is only made of this mapping
                group_trans_list_dict.pop(old_group_count,
                                          None)  # remove group
                trans_group_dict.pop(
                    read_id, None
                )  # remove read from trans_group_dict, will be re-assigned later
        ...
        if scaff_name == this_scaffold:
            ...
            if (start_pos >= group_start_pos
                    and start_pos <= group_end_pos):  # add to group
                group_trans_list_dict[group_count].append(read_id)
                trans_group_dict[read_id] = group_count
    ...
    for i in range(total_group_count + 1):
        ...
        if i not in group_trans_list_dict:
            ...
            print("""
                "This should only occur if you have a multi-map site that
                no reads are preferring."
            """)
        ...
        forward_gene_start_trans_dict, forward_start_gene_list = gene_group(
            forward_trans_list)
        reverse_gene_start_trans_dict, reverse_start_gene_list = gene_group(
            reverse_trans_list)
        ...
        for gene_start in all_start_list:
            ...
            # if a forward and reverse gene start at the same place use this to make
            # the the forward strand gene is represented first
            if gene_start in forward_gene_start_trans_dict:
                trans_id_list = list(
                    forward_gene_start_trans_dict[gene_start].keys())
                trans_obj_list = []
            ...
            if gene_start in reverse_gene_start_trans_dict:
                trans_id_list = list(
                    reverse_gene_start_trans_dict[gene_start].keys())
                trans_obj_list = []
                for trans_id in trans_id_list:
                    trans_obj_list.append(trans_obj_dict[trans_id])
                gene_trans_obj_list.append(trans_obj_list)

            # loop through list of trans obj lists, usually only one list since
            # unlikely for forward and reverse strand genes to coincide
            for trans_obj_list in gene_trans_obj_list:
                gene_count += 1

                # group transcripts by collapsability
                if fiveprime_cap_flag == "capped":
                    match_trans_group_dict, match_group_trans_dict = (
                        simplify_gene_capped(trans_obj_list,
                                             fiveprime_cap_flag))
                elif fiveprime_cap_flag == "no_cap":
                    match_trans_group_dict, match_group_trans_dict = (
                        simplify_gene_nocap(trans_obj_list,
                                            fiveprime_cap_flag))
                else:
                ...
                for match_group_num in match_group_trans_dict:
                    tmp_count += 1
                    tmp_trans_id = "G" + str(gene_count) + ".tmp." + str(
                        tmp_count)
                    merged_obj = Merged(tmp_trans_id)

                    match_trans_id_list = list(
                        match_group_trans_dict[match_group_num].keys())
                    match_trans_obj_list = []
                    ...
                    # if there are redundant transcripts, collapse
                    if len(match_trans_obj_list) > 1:
                        ...
                        exon_start_list = match_trans_obj_list[
                            0].exon_start_list
                        ...
                        for exon_index in range(
                                len(exon_start_list)):  # go from 3 prime end
                            (
                                e_start_priority,
                                e_end_priority,
                                e_start_priority_error,
                                e_end_priority_error,
                            ) = sj_error_priority_finder(
                                solo_trans_obj, exon_index, max_exon_num)

                            collapse_sj_start_err_list.append(e_start_priority)
                            collapse_sj_end_err_list.append(e_end_priority)

                            collapse_start_error_nuc_list.append(
                                e_start_priority_error)
                            collapse_end_error_nuc_list.append(
                                e_end_priority_error)
                ...
                for merged_obj in sorted_merge_obj_list:
                    trans_count += 1
                    final_trans_id = "G" + str(gene_count) + "." + str(
                        trans_count)
                    ...
                    num_junct_bind, bind_seq_dict = detect_rt_switch(
                        merged_obj)
                    if num_junct_bind > 0:
                    ...
                    for merged_trans_id in merged_obj.merged_trans_dict:
                        merged_trans_obj = merged_obj.merged_trans_dict[
                            merged_trans_id]
                        ...
                        exon_start_line, exon_end_line = (
                            merged_trans_obj.make_exon_start_end_lines())

                        merge_trans_bed_line = merged_trans_obj.format_bed_line(
                            final_trans_id)
                        # trans_clust_list = []
                        ...
                        downstream_seq = "".join(
                            merged_trans_obj.downstream_seq)
...
def process_variation(scaffold, variation_dict, var_coverage_dict,
                      var_support_threshold):

    cov_group_var_dict = {
    }  # cov_group_var_dict[cov group line][position line] = 1
    cov_group_var_list = []
    ...
    for var_pos in position_list:
        ...
        var_cov_trans_id_list = list(
            var_coverage_dict[scaffold][var_pos].keys())
        var_cov_trans_id_list.sort()
        var_coverage = len(var_cov_trans_id_list)

        # Use this to signal if a variation ahs passed threshold for this position
        var_pos_accept_flag = 0
        for var_type in var_type_list:
            ...
            for alt_seq in variation_dict[scaffold][var_pos][var_type]:
                ...
                read_list = list(variation_dict[scaffold][var_pos][var_type]
                                 [alt_seq].keys())
                ...
                if var_support_count >= var_support_threshold:
                    ...
                    # scaffold        position        type    alt_allele      count
                    # cov_count   cluster_list
                    var_outlist = []
...
def process_loci(this_trans_obj_dict, trans_list, this_gene_count):
    ...
    for gene_start in all_start_list:
        ...
        for trans_obj_list in gene_trans_obj_list:
            ...
            # group transcripts by collapsability
            if fiveprime_cap_flag == "capped":
                match_trans_group_dict, match_group_trans_dict = simplify_gene_capped(
                    trans_obj_list, fiveprime_cap_flag)
            elif fiveprime_cap_flag == "no_cap":
                match_trans_group_dict, match_group_trans_dict = simplify_gene_nocap(
                    trans_obj_list, fiveprime_cap_flag)
            else:
                print("Error with cap flag " + fiveprime_cap_flag)
                sys.exit()
            ...
            for match_group_num in match_group_trans_dict:
                tmp_count += 1
                tmp_trans_id = "G" + str(this_gene_count) + ".tmp." + str(
                    tmp_count)
                merged_obj = Merged(tmp_trans_id)
                ...
                if (len(match_trans_obj_list)
                        > 1):  # if there are redundant transcripts, collapse
                    (
                        collapse_start_list,
                        collapse_end_list,
                        start_wobble_list,
                    ...
                    for exon_index in range(
                        ...
                        (
                            ...
                            e_start_priority_error,
                            e_end_priority_error,
                        ) = sj_error_priority_finder(solo_trans_obj,
                                                     exon_index, max_exon_num)
            ...
            for merged_obj in sorted_merge_obj_list:
                trans_count += 1
                final_trans_id = "G" + str(this_gene_count) + "." + str(
                    trans_count)
                merged_obj.trans_id = final_trans_id
                print(final_trans_id)
                ...
                if num_junct_bind > 0:
                    for junct_num in bind_seq_dict:
                        "".join(bind_seq_dict[junct_num][1])
                        "".join(bind_seq_dict[junct_num][2])
                        "".join(bind_seq_dict[junct_num][3])
                ...
                for merged_trans_id in merged_obj.merged_trans_dict:
                    merged_trans_obj = merged_obj.merged_trans_dict[
                        merged_trans_id]
                    cluster_id = merged_trans_obj.cluster_id
                    merged_trans_obj.scaff_name
                    strand = merged_trans_obj.strand
                    merged_trans_obj.start_pos
                    merged_trans_obj.end_pos
...
if run_mode_flag == "low_mem":
    ...
    for line in sam_file_contents:
        ...
        [
            ...
            sj_pre_error_list,
            sj_post_error_list,
        ] = calc_error_rate_lowmem(start_pos, cigar, seq_list, scaff_name,
                                   read_id)
        ...
        if scaff_name == this_scaffold:
            ...
            if (start_pos >= group_start_pos
                ...
                # this is so we can get variation information
                # after confirming the read belongs to this group
                # 2020/07/31
            ...
            elif (start_pos > group_end_pos
                  ):  # start new group #################################

                # clear new read id from new group
                trans_obj_dict.pop(read_id, None)

                gene_count = process_loci(trans_obj_dict, group_trans_list,
                                          gene_count)  # 2020/07/30
                # if variation_dict:
                ...
                [
                    ...
                    sj_pre_error_list,
                    sj_post_error_list,
                ] = calc_error_rate_lowmem(start_pos, cigar, seq_list,
                                           scaff_name, read_id)
                ...
                trans_obj_dict = (
                    {}
                    # trans_obj_dict[cluster id] = trans obj # refresh this dict to save
                    # memory
                )
                trans_obj_dict[read_id] = trans_obj
    ...
    # this is not an issue with the original mode
    # because you collect groups as you go along.
    # but in low mem mode you only process a group
    # when a new group is found.
    gene_count = process_loci(trans_obj_dict, group_trans_list,
                              gene_count)  # 2020/07/30
    # if variation_dict:
    #    if len(group_trans_list) >= var_support_threshold:
        ...
        for var_pos in position_list:
            ...
            for var_type in var_type_list:
                ...
                for alt_seq in variation_dict[scaffold][var_pos][var_type]:
                    ...
                    read_list = list(variation_dict[scaffold][var_pos]
                                     [var_type][alt_seq].keys())
...
if run_mode_flag == "original":
    if multimap_missing_group_flag == 1:
        print("Missing group num, check for multi-maps in SAM file")
        print("""
            This should only occur if you have a multi-map site
            that no reads are preferring.
            """)
