...
This script cvalls variants from a bam/sam file.

"""

tc_version = "tvc0.0"
tc_date = "tvc_version_date_2021_07_09"
...
ap.add_argument(
    ...
    help=("""
        ...
        that is allowed (default is 1000 errors which practically means
...
ap.add_argument(
    ...
    nargs=1,
    help=("""
    Simple error symbol. Use this to pick the symbol used to represent matches
    in the simple error string for LDE output.
     """),
)
...
ap.add_argument(
    ...
    nargs=1,
    help=(
        'Clipped mapping variant calling: Default "clip_variants" and set to '
        '"no_clips_var" to ignore soft and hard clipping for variant detection'
    ),
)
...
if not opts.cv:
    print("Default clip_variants")
    clip_variant_flag = "clip_variants"
else:
    clip_variant_flag = str(opts.cv[0])
...
if sam_file.endswith("bam"):
    ...
    if bam_flag == "SAM":
        print("""
            You designated SAM input but are supplying BAM.
...
if sam_file.endswith("sam"):
    ...
    if bam_flag == "BAM":
        print("""
            You designated BAM input but are supplying SAM.
            Please use the same format for input as specified.
        """)
        sys.exit()
...
def calc_error_rate(start_pos, cigar, seq_list, scaffold, read_id):
    ...
    for i in range(len(cig_dig_list)):
        ...
        if cig_flag == "H":
            ...
            if clip_variant_flag == "clip_variants":
...
def calc_error_rate_lowmem(start_pos, cigar, seq_list, scaffold, read_id):
    ...
    for i in range(len(cig_dig_list)):
        ...
        if cig_flag == "H":
            ...
            #########################################
            # not related to sj error should probably move this
            # up or down code of this block
            #########################################
            intron_length = int(cig_dig_list[i])
            genome_pos = genome_pos + intron_length
            #########################################
...
def compare_transcripts(trans_obj, o_trans_obj, fiveprime_cap_flag,
    ...
    #        if(trans_obj.trans_id ==
    #        '1_14_c81301/2/1370' and o_trans_obj.trans_id ==
    #        '1_14_c147778/1/1455'):
    #            print("blah blah")
    #            print(trans_comp_flag)
    #            print(start_match_list)
    ...
    #            sys.exit()
    #        if(trans_obj.trans_id ==
    #        '1_14_c147778/1/1455' and o_trans_obj.trans_id ==
    #        '1_14_c81301/2/1370'):
    #            print("blah blah")
    #            print(trans_comp_flag)
    #           print(start_match_list)
    ...
    for trans_obj in trans_obj_list:
        ...
        if trans_pos_line in pos_trans_dict:
            ...
            if duplicate_flag == "no_merge":
                ...
                """)
                print("""
                    Duplicate transcript groups occur when different groupings
                    of transcripts results in the same collapsed model.
                """)
                print("""
                ...
                # collapse_start_list, collapse_end_list, start_wobble_list,
                # end_wobble_list, e_start_trans_dict,
                    ...
                    for prey_cluster_id in ungrouped_trans_obj_dict:
                        ...
                        if trans_match_flag != 1:
                            ...
                            if hunter_num_exons == prey_num_exons:
                                ...
                                #        transgroup.add_a_to_b_group(prey_cluster_id,
                                #            hunter_cluster_id)
                                #    # add shorter to longer
                                #    elif prey_num_exons > hunter_num_exons:
                                #        transgroup.add_a_to_b_group(hunter_cluster_id,
                                #            prey_cluster_id)

                                # remove the prey cluster from dict to
...
if run_mode_flag == "original":
    for line in sam_file_contents:
        ...
        if read_id in trans_obj_dict:
            ...
            if best_map_id == "B":
                ...
                # if this new map is not going to be used we can skip the rest of the
                # loop
                continue
    ...
    merged_obj_dict = {}
    # merged_obj_dict[final trans id] = merged obj
    gene_count = 0
...
def process_loci(this_trans_obj_dict, trans_list, this_gene_count):
    ...
    for gene_start in all_start_list:
        ...
        for trans_obj_list in gene_trans_obj_list:
            ...
            sort_transcripts(merge_obj_list)
...
if run_mode_flag == "original":
    ...
    for scaffold in scaffold_list:
        ...
        for var_pos in position_list:
            ...
            """
            Use this to signal if a variation ahs passed
            threshold for this position
            """
            var_pos_accept_flag = 0
            for var_type in var_type_list:
