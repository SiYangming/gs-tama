...
ap = argparse.ArgumentParser(
    description=("This script makes the Ensembl IDs the primary IDs "
...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help=('Filter level (default "none", use "only_match" '
          "to only include models with a match)"),
)
ap.add_argument(
    "-s",
    type=str,
    nargs=1,
    help=('Sub-field management method (default "ensembl_merge" '
          "for restructuring sub-fields from Ensembl ID, use "
          '"custom" to define sub-field shuffling)'),
)
ap.add_argument("-r",
                type=str,
                nargs=1,
                help='Sub-field reshuffle parameter (default "none")')
ap.add_argument("-d",
                type=str,
                nargs=1,
                help='Sub-field reshuffle delimiters (default ";")')
    ...
    if subfield_method == "ensembl_merge":
        ...
        if len(id_split) > 3:
            ...
            if "ENS" not in ens_gene_id:
                print("Ensembl gene ID is not right. "
                      "You may be using the wrong parameters. "
                      "Please see wiki for manual.")
                print(ens_gene_id)

                new_line = ("Ensembl gene ID is not right. "
                            "You may be using the wrong parameters. "
                            "Please see wiki for manual.")
            ...
            if "ENS" not in ens_trans_id:
                print("Ensembl transcript ID is not right. "
                      "You may be using the wrong parameters. "
                      "Please see wiki for manual.")
                print(ens_trans_id)

                new_line = ("Ensembl transcript ID is not right. "
                            "You may be using the wrong parameters. "
                            "Please see wiki for manual.")
                outfile.write(new_line)
                outfile.write("\n")
