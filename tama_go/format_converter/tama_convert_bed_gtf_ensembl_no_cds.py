...
#
# This script is used to convert the pacbio bed format file
# into a gtf file that mimics Ensembl's format
...
for line in bed_file_contents:
    ...
    trans_dict[trans_id] = Transcript(
        trans_id,
        gene_id,
        uniq_trans_id,
        chrom,
...
def format_trans_line(chrom, t_start, t_end, strand, gene_id, trans_id,
                      uniq_trans_id, source):
    # convert 0 coord bed to 1 coord gtf
    t_start = int(t_start) + 1

    anno_line = ('gene_id "' + gene_id + '"; transcript_id "' + trans_id +
                 '"; uniq_trans_id "' + uniq_trans_id + '";')
    outline = "\t".join([
        chrom,
        source,
        "transcript",
...
def format_exon_line(chrom, e_start, e_end, strand, gene_id, trans_id,
                     uniq_trans_id, source, e_num):
    # convert 0 coord bed to 1 coord gtf
    e_start = int(e_start) + 1

    anno_line = ('gene_id "' + gene_id + '"; transcript_id "' + trans_id +
                 '"; exon_number "' + str(e_num) + '"; uniq_trans_id "' +
                 uniq_trans_id + '";')
    outline = "\t".join([
        chrom, source, "exon",
        str(e_start),
        str(e_end), ".", strand, ".", anno_line
    ])
    return outline
...
for gene_id in gene_list:
    ...
    for trans_id in gene_trans_dict[gene_id]:
        ...
        outline = format_trans_line(chrom, t_start, t_end, strand, gene_id,
                                    trans_id, uniq_trans_id, source)
        outfile.write(outline)
        outfile.write("\n")
