...
ap.add_argument(
    ...
    help=("Level of removal (gene or transcript level). "
          "Gene level will only remove genes with a single read, "
          "transcript level will remove all singleton transcripts."),
)
...
ap.add_argument(
    ...
    type=str,
    nargs=1,
    help="Default to keep all multi-exon models (keep_multi or remove_multi)",
)
...
ap.add_argument(
    ...
    nargs=1,
    help=(
        "Requires models to have support from at least this number of sources. "
        "Default is 1"),
)
...
ap.add_argument(
    ...
    nargs=1,
    help=(
        "Requires models to have support from at least this number of reads. "
        "Default is 2"),
)
...
# test_files/data_remove_single_read_models_levels.txt

merge_trans_read_dict = {}
# merge_trans_read_dict[merge_trans_id][read id] = 1
merge_source_read_dict = {}
# merge_source_read_dict[mnerge trans id][source][read_id] = 1
...
for line in bed_file_contents:
    ...
    for source_name in merge_source_read_dict[trans_id]:
        this_source_num_reads = len(
            list(merge_source_read_dict[trans_id][source_name].keys()))
...
for gene_id in gene_list:
    ...
    if len(trans_list) == 1:
        ...
        if num_sources == 1:
            ...
            # num_reads = len(
            #     list(
            #         source_trans_read_dict[source_name][sub_trans_id].keys()
            #     )
            # )

            if num_reads == 1:
                if multi_flag == "keep_multi" and num_exons > 1:
                    # multi flag is for keeping multiple exon trans
                    output_flag = "keep"

                else:
                    # remove gene if it is single exon or if multi flag is set to remove
                    ...
                    source_line = ",".join(
                        list(merge_source_read_dict[trans_id].keys()))
    ...
    if filter_flag == "transcript":
        ...
        for trans_id in trans_list:
            ...
            if source_support_flag > 1:
                if num_sources >= source_support_flag:
                    ...
                    if multi_flag == "keep_multi" and num_exons > 1:
                        # multi flag is for keeping multiple exon trans
                        output_flag = "keep"
                    # remove if single exon or if multi flag is set to
                    # remove multi exon transcripts
                    else:
                        output_flag = "remove"
            ...
            if output_flag == "keep":
                ...
                print("keep " + trans_id + " " + str(total_trans_num_reads) +
                      " " + str(read_support_threshold) + " numexons " +
                      str(num_exons))
                # sys.exit()
        ...
        if source_support_flag > 1:
            ...
            for trans_id in trans_list:
                trans_split = gene_trans_dict[gene_id][trans_id]
                this_source_list = list(
                    merge_source_read_dict[trans_id].keys())
            ...
            if (
                    gene_num_sources < source_support_flag
            ):  # skip this gene if it does not meet number of sources requirement
                removed_gene_count += 1
                for trans_id in trans_list:
                    removed_trans_count += 1
