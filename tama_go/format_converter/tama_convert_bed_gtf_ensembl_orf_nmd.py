import sys
"""
This script is used to convert the pacbio bed format file into
a gtf file that mimics Ensembl's format
For use with bed files that are an output of tama orf/nmd prediction
pipeline includes CDS information
    ...
    def write_trans(self):
        ...
        for i in range(int(self.num_exons)):
            if self.strand == "+":
                ...
                if e_start > int(self.cds_end
                                 ):  # exon is completely 3' utr for pos strand
                ...
                if (int(self.cds_start)
                        > e_end):  # exon is completely 5' utr on pos strand
                    five_utr_start_list.append(e_start)
                    five_utr_end_list.append(e_end)
                    five_utr_num_list.append(e_num)
                    continue

                if e_start > int(self.cds_start) and e_end < int(
                        self.cds_end
                ):  # exon is within coding region but does not contain start or end
                    region_type = "CDS"
                    frame = "."
                    outline = format_exon_line(
                        region_type,
                ...
                if e_start < int(self.cds_end) and e_end >= int(
                        self.cds_end):  # cds end is within this exon
                    ...
                    cds_e_end = (
                        stop_codon_start - 1
                    )  # pos cds stop codon, cds does not includes it
                ...
                # cds start (start codon) is within this exon
                if int(self.cds_start) > e_start and int(
                        self.cds_start) <= e_end:
                    start_codon_start = self.cds_start
                    start_codon_end = int(self.cds_start) + 2

                    cds_e_start = (
                        start_codon_start  # pos cds start codon, cds includes it
                    )
                ...
                if int(self.cds_start) == int(
                        self.t_start
                ):  # no start codon because cds begins at beginning of transcript
                    start_codon_flag = 0
                ...
                if e_start > int(self.cds_end
                                 ):  # exon is completely 5' utr on neg strand
                    five_utr_start_list.append(e_start)
                    five_utr_end_list.append(e_end)
                    five_utr_num_list.append(e_num)
                    continue
                ...
                if (int(self.cds_start)
                        > e_end):  # exon is completely 3' utr for neg strand
                    three_utr_start = e_start
                    three_utr_start_list.append(three_utr_start)
                    three_utr_end = e_end
                    three_utr_end_list.append(three_utr_end)
                ...
                if e_start < int(self.cds_end) and e_end >= int(
                        self.cds_end
                ):  # cds end is within this exon neg strand
                    cds_e_end = int(
                        self.cds_end)  # neg cds start codon, cds includes it
                ...
                # cds start (stop codon) is within this exon
                if int(self.cds_start) > e_start and int(
                        self.cds_start) <= e_end:
...
def format_trans_line(chrom, t_start, t_end, strand, gene_id, trans_id,
                      source):
    # convert 0 coord bed to 1 coord gtf
    t_start = int(t_start)
...
for gene_id in gene_list:
    ...
    outline = format_gene_line(chrom, min_start, max_end, strand, gene_id,
                               source)
    outfile.write(outline)
    outfile.write("\n")
