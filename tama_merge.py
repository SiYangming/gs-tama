...
Added more informative error message for issues with

strand information from input bed files.
...
ap.add_argument("-a",
                type=str,
                nargs=1,
                help="5 prime threshold (Default is 10)")
ap.add_argument(
    "-m",
    type=str,
    nargs=1,
...
ap.add_argument("-z",
                type=str,
                nargs=1,
                help="3 prime threshold (Default is 10)")
...
source_dict = {
}  # source_dict[source id][filename, seq type, priority rank] = value
...
class Merged:
    ...
    def __init__(self, trans_id):
        ...
        self.merged_trans_dict = {
        }  # merged_trans_dict[source_trans id] = trans obj
        self.trans_list = []
        self.trans_obj_list = []
...
def compare_transcripts_both_capped(
        trans_obj, o_trans_obj, fivecap_flag, o_fivecap_flag,
    ...
    # if 5prime capped then should have same number of exons
    if diff_num_exon_flag == 1:
        ...
        for i in range(
                min_exon_num
        ):  # iterate from 3' end of transcript, strand corrected
            ...
            # set as default before deciding which thresholds to use
...
def compare_transcripts_capped_nocap(
        trans_obj, o_trans_obj, fivecap_flag, o_fivecap_flag,
        strand):  # use this to compare two transcripts
    ...
    if fivecap_flag == "capped" and o_fivecap_flag == "no_cap":
        ...
        print(
            "Error with cap flags, one needs to be capped and the other no cap"
        )
        print(fivecap_flag + " " + o_fivecap_flag)
        sys.exit()
    ...
    if trans_comp_flag != "diff_transcripts":
        ...
        # all_match_flag = 1
        # 1 if all matching and 0 if at least one not matching
        ...
        for i in range(
            ...
            if strand == "+":
                if i == 0:  # use three prime threshold if this is last exon
                    end_threshold = threeprime_threshold
                # use 5 prime threshold if this is first exon, never reaches
                # max_exon_num if diff exon num
                if i == max_exon_num - 1:
                    start_threshold = fiveprime_threshold
            ...
            start_match_flag, start_diff_num = fuzzy_match(
                c_e_start, n_e_start, start_threshold)
            end_match_flag, end_diff_num = fuzzy_match(c_e_end, n_e_end,
                                                       end_threshold)
            ...
            # Note that these occur at last loop, so no need ot break after match
            # decision
            elif (max_exon_num > min_exon_num and i
                  == min_exon_num - 1):  # diff num exons and at first min exon
                if strand == "+":
                    if end_match_flag == "no_match":
                        trans_comp_flag = "diff_transcripts"
                    elif start_match_flag == "no_match":
                        # if capped start is later than nocap start then no match
                        if c_e_start > n_e_start:
                            trans_comp_flag = "diff_transcripts"
                        # same three prime exons, with first no cap exon not
                        # as long as matching capped exon
                        else:
                    ...
                    if start_match_flag == "no_match":
                        ...
                        # for reverse strand, if capped start is not as long as nocap
                        # start then no match
                        if c_e_end < n_e_end:
                            trans_comp_flag = "diff_transcripts"
                        # same three prime exons, with first no cap exon not
                        # as long as matching capped exon
                            ...
                            trans_comp_flag = "same_three_prime_diff_exons"
                    # matches all exons for nocap but diff num exons
                    else:
                        trans_comp_flag = "same_three_prime_diff_exons"

            elif (max_exon_num == min_exon_num and i
                  == min_exon_num - 1):  # same number of exons and first exon
                if strand == "+":
                    if end_match_flag == "no_match":
                        trans_comp_flag = "diff_transcripts"
                    elif start_match_flag == "no_match":
    ...
    # so the match lists may not be as long as number of
    # exons if there is an internal no match
    # Keep in mind that the lists are ordered from 3' end to 5' end
    return (
        trans_comp_flag,
        start_match_list,
...
def compare_transcripts_both_nocap(
        trans_obj, o_trans_obj, fivecap_flag, o_fivecap_flag,
        strand):  # use this to compare two transcripts
    max_exon_num = 0
    min_exon_num = 0
    ...
    if len(e_start_list) != len(o_e_start_list):
        ...
        if len(e_start_list) > len(o_e_start_list):
            ...
            [
                ...
                max_exon_num,
                min_exon_num,
            ] = assign_short_long_trans(o_trans_obj, trans_obj)
            ...
            [
                ...
                max_exon_num,
                min_exon_num,
            ] = assign_short_long_trans(trans_obj, o_trans_obj)
    ...
    for i in range(min_exon_num
        ...
        if strand == "+":
            if i == 0:  # use three prime threshold if this is last exon
                end_threshold = threeprime_threshold
            # use 5 prime threshold if this is first exon, never reaches max_exon_num
            # if diff exon num
            if i == max_exon_num - 1:
                start_threshold = fiveprime_threshold
        ...
        start_match_flag, start_diff_num = fuzzy_match(l_e_start, s_e_start,
                                                       start_threshold)
        end_match_flag, end_diff_num = fuzzy_match(l_e_end, s_e_end,
                                                   end_threshold)
        #################################################################
        ...
        elif (max_exon_num > min_exon_num and i == min_exon_num -
              1):  # diff num exons and at first min exon, last loop
            if strand == "+":
                if end_match_flag == "no_match":
                    trans_comp_flag = "diff_transcripts"
                elif start_match_flag == "no_match":
                    # if capped start is later than nocap start then no match
                    if l_e_start > s_e_start:
                        trans_comp_flag = "diff_transcripts"
                    # same three prime exons, with first no cap exon not as long as
                    # matching capped exon
                    else:
                        trans_comp_flag = "same_three_prime_diff_exons"
                else:  # matches all exons for nocap but diff num exons
                    trans_comp_flag = "same_three_prime_diff_exons"
                ...
                if start_match_flag == "no_match":
                    ...
                    # for reverse strand, if capped start is not as long as nocap start
                    # then no match
                    if l_e_end < s_e_end:
                        trans_comp_flag = "diff_transcripts"
                    # same three prime exons, with first no cap exon not as long as
              ...
              1):  # same number of exons and first exon, last loop
            if strand == "+":
                if end_match_flag == "no_match":
                    trans_comp_flag = "diff_transcripts"
                elif start_match_flag == "no_match":
...
def collapse_transcripts(trans_obj_list,
    ...
    for i in range(max_exon_num):  # go from 3 prime end
        ...
        e_start_priority_dict = {
        }  # e_start_priority_dict[e start priority] = 1
        ...
        for trans_obj in trans_obj_list:
            ...
            # if this is the first 5' exon of group, use 5' start priority, and
            # junction end priority
            if i == max_exon_num - 1:
                if strand == "+":
            ...
            # if this is the first exon for this read and not the first for the
            # transcript group then set priority to last for 5' end of exon
            elif i < max_exon_num - 1 and i == len(e_start_list) - 1:
                # this situation should only happen with nocap reads
                if this_cap_flag != "no_cap":
            ...
            # if this is the 3' end of the transcript use end priority and junction
            # priority for start
            elif i == 0:
                if strand == "+":
                    e_start_priority = junction_priority
...
def gene_group(trans_obj_list,
    ...
    for i in range(len(trans_obj_list)):
        ...
        for j in range(i + 1, len(trans_obj_list)):
            ...
            if (uniq_trans_id in trans_gene_dict and o_uniq_trans_id
                    in trans_gene_dict):  # skip if already in same group
                if trans_gene_dict[uniq_trans_id] == trans_gene_dict[
                        o_uniq_trans_id]:
                    continue
            ...
            if overlap_flag == 0:  # no overlap make new gene groups
                ...
                if uniq_trans_id not in trans_gene_dict:
                    gene_count += 1
                    trans_gene_dict[uniq_trans_id] = gene_count
                ...
                # if no gene groups make new one
                if o_uniq_trans_id not in trans_gene_dict:
                    gene_count += 1
            ...
            if overlap_flag == 1:
                # if no gene groups make new one
                if (uniq_trans_id not in trans_gene_dict
                        and o_uniq_trans_id not in trans_gene_dict):
                    gene_count += 1
                    trans_gene_dict[uniq_trans_id] = gene_count
                    trans_gene_dict[o_uniq_trans_id] = gene_count
                    gene_trans_dict[gene_count] = {}
                    ...
                    if min_gene_start > o_exon_start_list[0]:
                        min_gene_start = o_exon_start_list[0]
                    gene_start_dict[gene_num] = min_gene_start
                elif (uniq_trans_id in trans_gene_dict
                      and o_uniq_trans_id in trans_gene_dict):

                    gene_num = trans_gene_dict[uniq_trans_id]
                    o_gene_num = trans_gene_dict[o_uniq_trans_id]

                    if gene_num != o_gene_num:  # merge gene groups
                        m_uniq_trans_id_list = list(
            ...
            for pos_element in same_order_index_list:
                ...
                for this_pos in slice_pos_index_list:
                    if past_pos == "na":
                        ...
                        print("Error with non-consequtive indices for "
...
def sort_transcripts(trans_obj_list, trans_obj_dict):
    ...
    for trans_obj in trans_obj_list:
        ...
        if trans_pos_line in pos_trans_dict:
            ...
            pos_trans_dict[trans_pos_line].trans_list
            trans_obj.trans_list
            ...
            print(list(
                pos_trans_dict[trans_pos_line].merged_trans_dict.keys()))
            ...
            if duplicate_flag == "no_merge":
                print("By default TAMA merge does not allow merging "
                      "of duplicate transcript groups.")
                print("Duplicate transcript groups occur when different "
                      "groupings of transcripts results in the same "
                      "collapsed model.")
                print("If you would like to merge duplicate transcript "
                      "groups please add -d merge_dup to the arguments.")
                sys.exit()
                ...
                (
                    ...
                    e_start_trans_dict,
                    e_end_trans_dict,
                ) = collapse_transcripts(match_trans_obj_list, collapse_flag)
...
def longest_transcript_old(trans_id, trans_id_list, trans_obj_dict):
    ...
    for o_trans_id in trans_id_list:
        ...
        if num_exons > o_num_exons:
            ...
            if strand == "+":
                # only allow exon diff threshold in case of nocap super long 5 prime
                # threshold
                start_match_flag, start_diff_num = fuzzy_match(
                    trans_start, o_trans_start, exon_diff_threshold)
                if start_diff_num <= 0:
                    longest_trans = "longest"
                elif start_match_flag != "no_match":
                    longest_trans = "longest"
                ...
                # only allow exon diff threshold in case of nocap super long 5 prime
                # threshold
                end_match_flag, end_diff_num = fuzzy_match(
                    trans_end, o_trans_end, exon_diff_threshold)
                if end_diff_num >= 0:
                    longest_trans = "longest"
                elif end_match_flag != "no_match":
                    longest_trans = "longest"
    ...
    if strand == "+":
        if num_exons > longest_num_exons:
            ...
            if trans_start < longest_tss_pos:
                ...
                print("Error with trans_start/longest_tss_pos "
                      "comparison in longest_transcript ")
                sys.exit()
        elif num_exons < longest_num_exons:
            longest_trans = "short"
        else:
...
def no_cap_check(
    this_trans_dict, trans_obj_dict
):  # this checks to see if all transcripts in a group are from no_cap
...
class TransGroup:
    ...
    def add_a_to_b_group(self, trans_a, trans_b, trans_obj_dict):
        ...
        for trans_c in self.group_trans_dict[b_group_num]:
            longest_trans_flag = longest_transcript(trans_c, b_trans_id_list,
                                                    trans_obj_dict)
    ...
    def add_a_to_b_group_both_nocap(self, trans_a, trans_b, trans_obj_dict):
        ...
        # add trans_a to b group and if a trans_a is longest nocap trans in it's
        # group add other nocap trans
        if len(list(self.trans_group_dict[trans_b].keys())) > 1:
            print("multiple groups")
            # sys.exit()
        ...
        for b_group_num in list(self.trans_group_dict[trans_b].keys()):
            ...
            for a_group_num in list(self.trans_group_dict[trans_a].keys()):
                ...
                print(
                    str(a_group_num) + "-a and b group num-" +
                    str(b_group_num))
                a_trans_id_list = list(
                    self.group_trans_dict[a_group_num].keys())
                a_longest_trans_flag = longest_transcript(
                    trans_a, a_trans_id_list, trans_obj_dict)
                ...
                if a_longest_trans_flag == "longest":
                    for a_group_trans in a_trans_id_list:
                        ...
                        self.trans_group_dict[a_group_trans].pop(
                            a_group_num, None)
            ...
            for trans_c in self.group_trans_dict[b_group_num]:
                longest_trans_flag = longest_transcript(
                    trans_c, b_trans_id_list, trans_obj_dict)
                self.trans_group_dict[trans_c][
                    b_group_num] = longest_trans_flag
                self.group_trans_dict[b_group_num][
                    trans_c] = longest_trans_flag  # continue here!!
    ...
    def add_a_to_b_group_nocap(self, trans_a, trans_b):
        ...
        # remove initial nocap group that is a self identity group
        # if only one group for trans_a
        if len(list(self.trans_group_dict[trans_a].keys())) == 1:
            a_trans_group = list(self.trans_group_dict[trans_a].keys())[0]
            # if log_flag == "log_on":
            #    print(trans_a)
            # if only one transcript in one group
            if len(list(self.group_trans_dict[a_trans_group].keys())) == 1:
                if (list(self.group_trans_dict[a_trans_group].keys())[0] ==
                        trans_a):  # if trans_a is just a group by itself
                    self.group_trans_dict.pop(a_trans_group, None)
                    self.trans_group_dict.pop(trans_a, None)
        ...
        # else:
        # this happens if trans_a is a nocap trans in
        # which case it can be in multiple groups
        # if log_flag == "log_on":
    ...
    def merge_a_b_groups_nocap_old(self, trans_a, trans_b, trans_obj_dict):
        ...
        if len(trans_id_list) > 0:  # new group was made
            ...
            for trans_c in self.group_trans_dict[self.group_count]:
                longest_trans_flag = longest_transcript(
                    trans_c, trans_id_list, trans_obj_dict)
                self.trans_group_dict[trans_c][
    ...
    def merge_a_b_groups_nocap(self, trans_a, trans_b, trans_obj_dict):
        ...
        if len(trans_id_list) > 0:  # new group was made
            ...
            for check_trans_id in trans_id_list:
                longest_trans_flag = longest_transcript(
                    check_trans_id, [longest_trans_id], trans_obj_dict)
            ...
            for trans_c in trans_id_list:
                longest_trans_flag = longest_transcript(
                    trans_c, [longest_trans_id], trans_obj_dict)
                self.trans_group_dict[trans_c][
                    self.group_count] = longest_trans_flag
                self.group_trans_dict[
                    self.group_count][trans_c] = longest_trans_flag
...
def hunter_prey_capped(capped_trans_obj_list, trans_obj_dict, transgroup):
    ...
    while ungrouped_count > 0:
        ...
        if unsearched_count == 0:
            ...
            hunter_fivecap_flag = source_dict[
                hunter_trans_obj.source_id]["seq_type"]
        ...
        while unsearched_count > 0 and ungrouped_count > 0:
            ...
            for prey_cluster_id in ungrouped_trans_obj_dict:
                ...
                prey_fivecap_flag = source_dict[
                    prey_trans_obj.source_id]["seq_type"]
                ...
                (
                    ...
                    short_trans,
                    min_exon_num,
                ) = compare_transcripts_both_capped(
                    hunter_trans_obj,
                    prey_trans_obj,
                    hunter_fivecap_flag,
                    prey_fivecap_flag,
...
def hunter_prey_nocap(nocap_trans_obj_list, trans_obj_dict, transgroup):
    ...
    used_hunter_dict = (
        {}
        # used_hunter_dict[hunter_id] = 1  # use this to skip hunters that have
        # already been used
    )
    ...
    for trans_obj in nocap_trans_obj_list:
        ...
        exon_trans_obj_dict[this_num_exons][
            trans_obj.uniq_trans_id] = trans_obj
    ...
    while exon_num_index < len(exon_num_list):
        ...
        while ungrouped_count > 0:
            ...
            if unsearched_count == 0:
                ...
                print("hunter id unsearched_count: " +
                      str(hunter_cluster_id))  # troubleshooting
            ...
            while unsearched_count > 0:
                ...
                if hunter_cluster_id == "new_hunter":
                    ...
                    this_ungrouped_trans_obj_list = list(
                        ungrouped_trans_obj_dict.keys())

                    print("hunter id new_hunter: " +
                          str(hunter_cluster_id))  # troubleshooting
                ...
                if hunter_cluster_id not in sub_length_cluster_dict:
                    for prey_cluster_id in ungrouped_trans_obj_dict:
                        ...
                        (
                            ...
                            short_trans,
                            min_exon_num,
                        ) = compare_transcripts_both_nocap(
                            hunter_trans_obj,
                            prey_trans_obj,
                            hunter_fivecap_flag,
                            prey_fivecap_flag,
                        ...
                        if trans_match_flag != 1:  # skip if there is no match
                            ...
                            if hunter_num_exons == prey_num_exons:
                                ...
                                if trans_comp_flag == "same_transcript":  # 2019/06/07
                                    ...
                                    transgroup.add_a_to_b_group_nocap(
                                ...
                                # make sure to only add unsearched prey if it has not
                                # already been used as a hunter
                                if prey_cluster_id not in used_hunter_dict:
                                    unsearched_trans_obj_dict[
                                        prey_cluster_id] = (prey_trans_obj)
                ...
                while prey_exon_index < len(exon_num_list):
                    ...
                    for prey_cluster_id in subgrouped_trans_obj_dict:
                        ...
                        # trans_comp_flag, start_match_list, start_diff_list,
                        # end_match_list, end_diff_list, short_trans, long_trans,
                        ...
                        if trans_comp_flag == "same_transcript":
                            ...
                            # trans_match_flag = 1
                            print(
                                "Error with subgroup search same_three_prime_same_exons"
                            )
                            sys.exit()
                        elif trans_comp_flag == "same_three_prime_diff_exons":
                            trans_match_flag = 1
                        ...
                        if trans_match_flag != 1:  # skip if there is no match
                            ...
                            if hunter_num_exons == prey_num_exons:
                                ...
                                print("sub_length_cluster_dict : hunter - " +
                                      hunter_cluster_id + "; prey - " +
                                      prey_cluster_id)
...
def hunter_prey_mixed(capped_trans_obj_list, nocap_trans_obj_list,
                      trans_obj_dict, transgroup):
    # new cluster grouping algorithm
    ##########################################################################
    ###########################
    # convert capped_trans_obj_list to trans_obj_dict for while looping
    ...
    # clusters that have yet been grouped
    pass
    # ungrouped_trans_obj_dict[uniq_trans_id] =  trans_obj
    # clusters that have been grouped
    # grouped_trans_obj_dict = {}
    # grouped_trans_obj_dict[uniq_trans_id] =  trans_obj
    # grouped clusters that have been not been searched/been used as a hunter
    # unsearched_trans_obj_dict[uniq_trans_id] =  trans_obj
    ...
    for hunter_trans_obj in capped_trans_obj_list:
        ...
        for prey_trans_obj in nocap_trans_obj_list:
            ...
            # skip this if hunter and prey are already associated
            if group_match_flag == 1:
                continue
            ...
            (
                ...
                short_trans,
                min_exon_num,
            ) = compare_transcripts_capped_nocap(
                hunter_trans_obj,
                prey_trans_obj,
                hunter_fivecap_flag,
                prey_fivecap_flag,
            ...
            if trans_match_flag != 1:  # skip if there is no match
                ...
                # same number of exons or fewer for prey
                if hunter_num_exons >= prey_num_exons:
...
def simplify_gene(
    ...
    if len(capped_trans_obj_list) > 0:
        transgroup = hunter_prey_capped(capped_trans_obj_list, trans_obj_dict,
                                        transgroup)
    ############################################
    # compare capped groups with no-cap transcripts

    if len(capped_trans_obj_list) > 0 and len(nocap_trans_obj_list) > 0:
        transgroup = hunter_prey_mixed(capped_trans_obj_list,
                                       nocap_trans_obj_list, trans_obj_dict,
                                       transgroup)
    ...
    transgroup = hunter_prey_nocap(ungrouped_nocap_trans_obj_list,
                                   trans_obj_dict, transgroup)
...
def format_gene_report_line(trans_obj_list, total_gene_count,
                            total_final_trans_count, track_gene_source):
...
def process_trans_group(trans_line_list, total_gene_count):
    ...
    for trans_line_split in trans_line_list:
        ...
        if len(id_line_split) < 2:
            ...
            print(
                "bed12 files must have the gene ID's and transcript ID's "
                'formatted as such "gene_id;transcript_id" in the 4th column.')
            print(
                "The gene ID must be the first subfield and the subfields must "
                "be delimited with a semicolon (;).")
            sys.exit()
        ...
        [start_priority, junction_priority,
         end_priority] = source_dict[source_id]["priority_rank"].split(",")
        ...
        if trans_obj.strand == "+":
            ...
            this_error_line = ("Source: " + issue_source + " Trans: " +
                               issue_trans_id + " Strand: " + issue_strand)
    ...
    # Python3 兼容：dict_keys 无 sort() 方法，改为使用 sorted()
    ...
    for gene_start in all_start_list:
        ...
        if gene_start in forward_gene_start_trans_dict:
            uniq_trans_id_list = forward_gene_start_trans_dict[
                gene_start].keys()
            trans_obj_list = []
        ...
        for trans_obj_list in gene_trans_obj_list:
            ...
            match_trans_group_dict, match_group_trans_dict = simplify_gene(
                trans_obj_list, trans_obj_dict)
            ...
            for match_group_num in match_group_trans_dict:
                tmp_count += 1
                tmp_trans_id = "G" + str(total_gene_count) + ".tmp." + str(
                    tmp_count)
                ...
                if (len(match_trans_obj_list)
                    ...
                    for z in range(len(exon_start_list)):
                        ...
                        e_start_trans_dict[exon_start_list[z]][
                            match_trans_obj_list[0].uniq_trans_id] = 1
            ...
            track_gene_source_dict = {
            }  # track_gene_source_dict[source_gene_id] = 1

            trans_count = 0
            for merged_obj in sorted_merge_obj_list:
                trans_count += 1
                final_trans_id = "G" + str(total_gene_count) + "." + str(
                    trans_count)
                ...
                for merged_trans_id in merged_obj.merged_trans_dict:
                    ...
                    if len(track_merge_gene_id_split) > 2:
                        ...
                        for t_index in range(
                                len(track_merge_gene_id_split) - 1):
                            track_merge_gene_id_list.append(
                                track_merge_gene_id_split[t_index])

                        track_merge_gene_id = ".".join(
                            track_merge_gene_id_list)
                    else:
                    ...
                    if source_id_flag != "no_source_id":
                        ...
                        for this_source_id_flag in source_id_flag_list:
                            if this_source_id_flag == this_source_name:
                                this_source_gene_id = source_trans_gene_dict[
                                    this_source_name][this_source_trans_id]

                                if merged_obj.extra_trans_id_list == "NA":
                                    merged_obj.extra_trans_id_list = []
                                    merged_obj.extra_gene_id_list = []

                                merged_obj.extra_trans_id_list.append(
                                    this_source_trans_id)
                                merged_obj.extra_gene_id_list.append(
                                    this_source_gene_id)

                    # add CDS information from a source file
                    # AXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
                    if source_cds_flag != "no_cds":
                        this_source_name_split = merged_trans_id.split("_")
                        this_source_name = this_source_name_split[0]
            ...
            gene_report_line = format_gene_report_line(trans_obj_list,
                                                       total_gene_count,
                                                       trans_count,
                                                       track_gene_source)
            outfile_gene_report.write(gene_report_line)
...
##########################################################################
##########################################################################
source_trans_gene_dict = {
}  # source_trans_gene_dict[source name][trans_id] = gene_id
...
for file_line in filelist_file_contents:
    ...
    if "\r" in file_line:
        ...
        print(file_line)
        print(
            "Please make sure the filelist file does not have DOS new line characters"
        )
        sys.exit()
    ...
    if len(space_line_split) > 1:
        ...
        print(file_line)
        print(
            "Please make sure it is tab separated with no empty lines and no spaces"
        )
        sys.exit()
    ...
    if source_id not in source_dict:
        ...
        # by type of merge event 1,1,1 ->  start,junction,end
        source_dict[source_id]["priority_rank"] = priority_rank

        if seq_type != "capped" and seq_type != "no_cap":
            print("Incorrect seq types given in filelist file. "
                  "Please use capped or no_cap")
            sys.exit()
    ...
    for line in bed_file_contents:
        ...
        if len(id_line_split) < 2:
            ...
            print(filename)
            print(line)
            print("bed12 files must have the gene ID's and "
                  "transcript ID's formatted as such "
                  '"gene_id;transcript_id" in the 4th column.')
            print("The gene ID must be the first subfield and "
                  "the subfields must be delimited with a semicolon (;).")
            sys.exit()
...
if source_id_flag != "no_source_id":
    for this_source_id_flag in source_id_flag_list:
        if this_source_id_flag not in source_dict:
            print("Error: Source name given in -n parameter is "
                  "not found in filelist file.")
    ...
    for trans_start in trans_start_list:
        ...
        if trans_start < last_trans_end:  # matches previous group
            ...
            for trans_end in trans_end_list:
                ...
                group_trans_list.extend(
                    bed_dict[scaffold][trans_start][trans_end])
                if last_trans_end < trans_end:  # update last trans end
                    last_trans_end = trans_end

        elif (trans_start
              >= last_trans_end):  # if this is a new group, process past group

            total_gene_count = process_trans_group(group_trans_list,
                                                   total_gene_count)
