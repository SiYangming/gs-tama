...
# This script looks at read support files and finds reads that have mapped
# to different genes

ap = argparse.ArgumentParser(
    description=("This script looks at read support files "
                 "and finds reads that have mapped to different genes"))
...
read_source_gene_dict = {
}  # read_source_gene_dict[read_id][source][gene_id] = 1
read_gene_dict = {}  # read_gene_dict[read_id][gene_id] = 1

read_source_trans_dict = {
}  # read_source_trans_dict[read_id][source][trans_id] = 1
...
source_gene_merge_dict = {
}  # source_gene_merge_dict[source][gene_id] = merge_gene_id
source_trans_merge_dict = (
    {})  # source_trans_merge_dict[source][trans_id] = merge_trans_id
...
outline = "\t".join([
    "read_id",
    "alt_trans_diff_count",
    "alt_diff_trans_id_list_line",
    "alt_trans_id_list_line",
    "ref_trans_id_list_line",
])
...
for read_id in all_read_list:
    ...
    if num_genes > 1:
        ...
        outline = "\t".join([
            read_id,
            str(num_genes), all_gene_line, all_pos_line, all_trans_line
        ])
        outfile_gene.write(outline)
        outfile_gene.write("\n")
...
for this_source in all_source_list:
    this_source_diff_genes = len(
        list(diff_gene_source_dict[this_source].keys()))
    this_source_diff_trans = len(
        list(diff_trans_source_dict[this_source].keys()))

    outline = ("this_source_diff_genes " + str(this_source) + ": " +
               str(this_source_diff_genes))
    outfile_report.write(outline)
    outfile_report.write("\n")

    outline = ("this_source_diff_trans " + str(this_source) + ": " +
               str(this_source_diff_trans))
    outfile_report.write(outline)
    outfile_report.write("\n")
...
for merge_trans_id in merge_diff_trans_dict:
    ...
    if num_sources == 2:
        ...
        # only count one source trans if it is not involved in a one source gene!
...
for this_source in all_source_list:
    only_source_num_genes = len(list(
        only_source_gene_dict[this_source].keys()))
    only_source_num_trans = len(
        list(only_source_trans_dict[this_source].keys()))
    ...
    outline = ("only_source_num_genes " + str(this_source) + ": " +
               str(only_source_num_genes))
    outfile_report.write(outline)
    outfile_report.write("\n")

    outline = ("only_source_num_trans " + str(this_source) + ": " +
               str(only_source_num_trans))
    outfile_report.write(outline)
    outfile_report.write("\n")
