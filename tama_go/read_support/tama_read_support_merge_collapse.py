...
gene_trans_read_dict = (
    {})  # gene_trans_read_dict[source][gene id][trans id][read id] = 1
prefix_list = []

for fline in filelist_file_contents:
    fline_split = fline.split("\t")

    # read_support_tama_collapse_oldbrain_nocap.txt    oldbrain.
    # /exports/cmvm/eddie/eb/groups/burt_chicken_isoseq/dev_rk/4_tama_collapse_all/1_oldbrain/
    # read_support_tama_collapse_embryo.txt   embryo
    # /exports/cmvm/eddie/eb/groups/burt_chicken_isoseq/dev_rk/4_tama_collapse_all/2_embryo/
    ...
    for line in support_file_contents:
        ...
        # G1      G1.1    3       3
        # rec_c6109:m54041_161210_051053/74842291/1157_29_CCS,m54041_161209_145041/37749615/30_1156_CCS,m54041_161210_051053/56034011/1128_31_CCS
...
merge_gene_trans_read_dict = (
    {}
    # merge_gene_trans_read_dict[merge gene][merge trans][prefix][source
    # trans][read id] = 1
)

merge_gene_trans_dict = {
}  # merge_gene_trans_dict[merge gene] = list of merge trans
...
for line in merge_file_contents:
    ...
    # 1       219     3261    G1.1;spleen_G1.1        40      +       219
    # 3261    255,0,0 5       98,93,181,107,714       0,1457,1757,2132,2328
    ...
    merge_read_dict[trans_id][
        prefix] = merge_read_dict[trans_id][prefix] + int(read_count)
    ...
    if collapse_id not in merge_gene_trans_read_dict[gene_id][trans_id][
            prefix]:
        merge_gene_trans_read_dict[gene_id][trans_id][prefix][collapse_id] = {}

    source_gene_id = collapse_id.split(".")[0]
    for support_read_id in gene_trans_read_dict[prefix][source_gene_id][
            collapse_id]:
        merge_gene_trans_read_dict[gene_id][trans_id][prefix][collapse_id][
            support_read_id] = 1
...
for merge_gene_id in merge_gene_list:
    ...
    for merge_trans_id in merge_gene_trans_read_dict[merge_gene_id]:
        for this_prefix in merge_gene_trans_read_dict[merge_gene_id][
                merge_trans_id]:
            for support_trans_id in merge_gene_trans_read_dict[merge_gene_id][
                    merge_trans_id][this_prefix]:
                for support_read_id in merge_gene_trans_read_dict[
                        merge_gene_id][merge_trans_id][this_prefix][
                            support_trans_id]:
    ...
    for merge_trans_id in merge_gene_trans_dict[merge_gene_id]:
        ...
        for this_prefix in merge_gene_trans_read_dict[merge_gene_id][
            ...
            for support_trans_id in merge_gene_trans_read_dict[merge_gene_id][
                ...
                this_trans_support_read_list = list(
                    merge_gene_trans_read_dict[merge_gene_id][merge_trans_id]
                    [this_prefix][support_trans_id].keys())
                this_trans_support_read_line = ",".join(
                    this_trans_support_read_list)
        ...
        outline = "\t".join([
            merge_gene_id,
            merge_trans_id,
            str(total_read_count_gene),
            str(total_read_count_trans),
            this_prefix_line,
            this_support_trans_line,
            this_all_support_read_line,
        ])
        outfile.write(outline)
        outfile.write("\n")
