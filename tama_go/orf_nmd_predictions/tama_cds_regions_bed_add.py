...
#
# This script uses data from the blastp parse file and
# the pacbio annotation to assign the locations of the
# UTR/CDS regions to the bed file
# Use sequence file to double check the sequence lengths
# Last updated 2020/12/11
#

ap = argparse.ArgumentParser(
    description=("This script uses data from the blastp parse file and "
                 "the original annotation to assign the locations of the "
                 "UTR/CDS regions to the bed file"))
...
ap.add_argument("-f",
                type=str,
                nargs=1,
                help="Fasta for annotation file (required)")
ap.add_argument("-o", type=str, nargs=1, help="Output file name (required)")
ap.add_argument(
    "-s",
    type=str,
    nargs=1,
    help=("Include stop codon in CDS region (include_stop), "
          "default is to remove stop codon from CDS region"),
)
ap.add_argument(
    "-d",
    type=str,
    nargs=1,
    help=("Distance from last splice junction to call NMD (default 50bp)"),
)
...
if matching_trans_id_count == 0:
    ...
    print(
        "Check the ID's in all the input files to see if they have matching formats."
    )
    sys.exit()
...
for line in pbri_file_contents:
    ...
    if trans_id not in trans_dict:
        ...
        line_split[3] = (line_split[3] + ";" + prot_id + ";" + degrade_flag +
                         ";" + match_flag + ";" + nmd_flag + ";" + frame)
    ...
    if strand == "+":
        ...
        # cds_rel_end =  prot_end * 3 + pos_adjust + 3  # minus 1 to adjust to bed
        # coords
        ...
        if include_stop_flag == "include_stop":
            cds_rel_start = trans_len_dict[trans_id] - (nuc_end + pos_adjust +
                                                        1)
        else:
            cds_rel_start = trans_len_dict[trans_id] - (nuc_end + pos_adjust -
                                                        2)
    ...
    for i, block_size in enumerate(block_list):
        ...
        if cds_rel_start >= prev_block_sum and cds_rel_start < block_sum:
            exon_cds_start = i
            cds_coord_start = (trans_start + int(block_start_list[i]) +
                               cds_rel_start - prev_block_sum)
        if cds_rel_end >= prev_block_sum and cds_rel_end <= block_sum:
            exon_cds_end = i
            cds_coord_end = (trans_start + int(block_start_list[i]) +
                             cds_rel_end - prev_block_sum)
    ...
    if block_sum != trans_len_dict[trans_id]:
        ...
        print(trans_id + "\t" + str(block_sum) + "\t" +
              str(trans_len_dict[trans_id]))
        sys.exit()
